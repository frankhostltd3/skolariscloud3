...
return new class extends Migration
{
    ...
    {
        if (!Schema::hasTable('tenant_payment_transactions')) {
            Schema::create('tenant_payment_transactions', function (Blueprint $table) {
                $table->id();
                $table->string('transaction_type')->default('fee'); // 'fee' or other tenant payments
                $table->unsignedBigInteger('related_id'); // fee_payment_id, etc.
