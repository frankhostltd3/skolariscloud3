...
$perm = DB::connection('mysql')->table('permissions')->where('name', $permName)->where('guard_name', 'landlord')->first();

if ($perm) {
    echo "Updating Permission '{$permName}' tenant_id to 'landlord'...\n";
    DB::connection('mysql')->table('permissions')
        ->where('id', $perm->id)
        ->update(['tenant_id' => 'landlord']);
} else {
    echo "Creating Permission '{$permName}'...\n";
    $permId = DB::connection('mysql')->table('permissions')->insertGetId([
        'name' => $permName,
        'guard_name' => 'landlord',
        'tenant_id' => 'landlord',
        'created_at' => now(),
...
$role = DB::connection('mysql')->table('roles')->where('name', $roleName)->where('guard_name', 'landlord')->first();

if ($role) {
    echo "Updating Role '{$roleName}' tenant_id to 'landlord'...\n";
...
$hasRole = DB::connection('mysql')->table('model_has_roles')
...
if (!$hasRole) {
    ...
    echo "User already has role. Updating tenant_id to 'landlord'...\n";
...
$roleHasPerm = DB::connection('mysql')->table('role_has_permissions')
...
$userHasPerm = DB::connection('mysql')->table('model_has_permissions')
    ->where('model_id', $user->id)
    ->where('model_type', get_class($user))
    ->where('permission_id', $perm->id)
    ->exists();
...
if (!$userHasPerm) {
    ...
    echo "User already has permission. Updating tenant_id to 'landlord'...\n";
