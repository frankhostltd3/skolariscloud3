...
class UserApprovalController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($status === 'approved') {
            try {
                $classes = ClassRoom::orderBy('grade_level')->orderBy('name')->get(['id','name','code']);
            } catch (\Throwable $e) {
    ...
    public function updateEmployment(Request $request, User $user)
    {
        ...
        DB::transaction(function () use ($user, $validated) {
            ...
            if ($validated['employment_role'] === 'Teacher') {
                try { $user->assignRole('Teacher'); } catch (\Throwable $e) { /* ignore */ }
     ...
     */
    private function createStudentRecord(User $user)
    {
        ...
        $admissionNo = $registrationData['student_id'] 
        ...
        \App\Models\Student::create([
            'name' => $user->name,
            'email' => $user->email,
            'phone' => $user->phone ?? $registrationData['phone'] ?? '',
            'admission_no' => $admissionNo,
            'admission_date' => now(),
            'status' => 'active',
            'gender' => $registrationData['gender'] ?? 'other',
            'class_id' => $registrationData['class_id'] ?? null,
            'stream_id' => $registrationData['stream_id'] ?? null,
            ...
            'guardian_email' => $registrationData['guardian_email'] ?? '',
        ]);
    }
