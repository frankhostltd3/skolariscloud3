...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <form action="{{ route('landlord.landing-testimonials.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('role') is-invalid @enderror" id="role"
                        name="role" value="{{ old('role') }}" placeholder="e.g., Principal, Greenwood High">
                    @error('role')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('rating') is-invalid @enderror" id="rating"
                        name="rating" value="{{ old('rating', 5) }}" min="1" max="5" required>
                    @error('rating')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('avatar_url') is-invalid @enderror" id="avatar_url"
                        name="avatar_url" value="{{ old('avatar_url') }}">
