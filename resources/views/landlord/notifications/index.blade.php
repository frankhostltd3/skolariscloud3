...
@section('content')
    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3 mb-4">
        <div>
            ...
            <p class="text-secondary mb-0">{{ __('Create, schedule, and view notifications to tenants or internal users.') }}
            </p>
        </div>
        <div class="d-flex align-items-center gap-2">
            <a href="{{ route('landlord.notifications.create') }}" class="btn btn-primary btn-sm">
                    ...
                    <select name="channel" class="form-select">
                        ...
                        @foreach (['system', 'email', 'sms', 'slack', 'webhook'] as $ch)
                            <option value="{{ $ch }}" @selected(request('channel') === $ch)>
                                {{ \Illuminate\Support\Str::upper($ch) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-sm-6 col-lg-3">
                ...
                <tbody>
                    @forelse ($notifications as $n)
                        <tr>
                            ...
                            <td><span
                                    class="badge text-bg-light text-body border">{{ \Illuminate\Support\Str::upper($n->channel) }}</span>
                            </td>
                            <td class="text-secondary small">{{ optional($n->scheduled_at)->format('Y-m-d H:i') ?? '—' }}
                            </td>
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item disabled" href="#"><span
                                                    class="bi bi-eye me-2"></span>{{ __('View') }}</a></li>
                                        <li><a class="dropdown-item disabled" href="#"><span
                                                    class="bi bi-pencil me-2"></span>{{ __('Edit') }}</a></li>
                                    </ul>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="text-center text-secondary py-5">{{ __('No notifications found.') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
