...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <form action="{{ route('landlord.landing-sections.update', $landing_section->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                        name="name" value="{{ old('name', $landing_section->name) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('component') is-invalid @enderror" id="component"
                        name="component" value="{{ old('component', $landing_section->component) }}" required>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" class="form-control @error('sort_order') is-invalid @enderror" id="sort_order"
                            name="sort_order" value="{{ old('sort_order', $landing_section->sort_order) }}">
                        @error('sort_order')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('is_active') is-invalid @enderror" id="is_active"
                            name="is_active">
                            <option value="1"
                                {{ old('is_active', $landing_section->is_active) == 1 ? 'selected' : '' }}>Active</option>
                            <option value="0"
                                {{ old('is_active', $landing_section->is_active) == 0 ? 'selected' : '' }}>Inactive</option>
                        </select>
                        @error('is_active')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
