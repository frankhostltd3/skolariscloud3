    ...
    <body class="bg-body-tertiary text-body">
        ...
        <header class="border-bottom bg-white shadow-sm">
            <nav class="navbar navbar-expand-lg py-3">
                <div class="container-xxl">
                    <a class="navbar-brand d-flex align-items-center gap-2 fw-semibold" href="{{ route('landlord.dashboard') }}">
                ...
                @php
                    if (! isset($navigation)) {
                        $navigation = [
                            ...
                            [ 'label' => __('Create tenant'), 'icon' => 'bi-building-add', 'route' => 'landlord.tenants.create' ],
                            ...
                            [ 'label' => __('Pricing catalogue'), 'icon' => 'bi-tags', 'route' => 'landlord.billing.plans.index', 'pattern' => 'landlord.billing.plans.*' ],
                            [ 'label' => __('Invoices'), 'icon' => 'bi-receipt', 'route' => 'landlord.billing.invoices' ],
                            [ 'label' => __('Payment methods'), 'icon' => 'bi-wallet2', 'route' => 'landlord.billing.payment-methods' ],
                            [ 'label' => __('Dunning'), 'icon' => 'bi-exclamation-triangle', 'route' => 'landlord.billing.dunning' ],
                            ...
                            [ 'label' => __('Notifications'), 'icon' => 'bi-bell', 'route' => 'landlord.notifications.index' ],
                                ...
                                <nav class="nav flex-column">
                                    @foreach ($navigation as $item)
                                        @if (!empty($item['children']))
                                            ...
                                            @endphp
                                            <button class="btn text-start d-flex align-items-center gap-2 px-3 py-2 w-100 @if($childActive) text-primary fw-semibold @else text-secondary @endif" type="button" data-bs-toggle="collapse" data-bs-target="#{{ $groupId }}" aria-expanded="{{ $childActive ? 'true' : 'false' }}" aria-controls="{{ $groupId }}">
                                                <span class="bi {{ $item['icon'] }} nav-icon"></span>
                                                <span>{{ $item['label'] }}</span>
                                                <span class="ms-auto bi @if($childActive) bi-chevron-up @else bi-chevron-down @endif"></span>
                                            </button>
                                            <div class="collapse @if($childActive) show @endif" id="{{ $groupId }}">
                                                <div class="d-flex flex-column ms-4 border-start ps-2">
                                                    @foreach ($item['children'] as $child)
                                                        ...
                                                        @endphp
                                                        <a href="{{ route($child['route']) }}" class="nav-link d-flex align-items-center gap-2 px-3 py-2 @if($isActive) active text-primary fw-semibold @else text-secondary @endif">
                                        ...
                                        @else
                                            @php
                                                ...
                                                $isActive = request()->routeIs($pattern);
                                            @endphp
                                            <a href="{{ route($item['route']) }}" class="nav-link d-flex align-items-center gap-2 px-3 py-2 @if($isActive) active text-primary fw-semibold @else text-secondary @endif">
                                                <span class="bi {{ $item['icon'] }} nav-icon"></span>
                                                <span>{{ $item['label'] }}</span>
                                            </a>
                                        @endif
        ...
        <footer class="py-4 bg-body-secondary text-secondary small">
            <div class="container-xxl d-flex flex-column flex-md-row align-items-center justify-content-between gap-3">
                <span>&copy; {{ now()->year }} {{ config('app.name') }}. {{ __('Multi-tenant orchestration for schools.') }}</span>
                <div class="d-flex align-items-center gap-3">
                    <a class="link-secondary text-decoration-none" href="{{ route('landing', ['locale' => app()->getLocale()]) }}">{{ __('Marketing site') }}</a>
                    <a class="link-secondary text-decoration-none" href="mailto:support@skolariscloud.com">{{ __('Support') }}</a>
                </div>
            </div>
        </footer>
