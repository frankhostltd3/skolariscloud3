                ...
                <div class="card-body d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <div class="fs-3 fw-bold">
                            {{ $stats['average_attendance'] !== null ? number_format($stats['average_attendance'], 1) . '%' : __('N/A') }}
                        </div>
            ...
            <div class="card stats-card h-100">
                <div class="card-body d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <div class="fs-3 fw-bold">
                            {{ number_format(($stats['unread_messages'] ?? 0) + ($stats['unread_notifications'] ?? 0)) }}
                        </div>
                            ...
                            <div class="border rounded-4 h-100 p-3">
                                <div class="d-flex flex-column flex-sm-row justify-content-between gap-3 mb-3">
                                    ...
                                    <div class="text-sm-end">
                                        @if ($averageGrade !== null)
                                            <span class="badge bg-success bg-opacity-10 text-success">{{ __('Avg') }}
                                                {{ number_format($averageGrade, 1) }}%</span>
                                        @else
                                            <span
                                                class="badge bg-secondary bg-opacity-10 text-secondary">{{ __('No grades yet') }}</span>
                                        @endif
                                        @if ($ward['class_teacher'])
                                            <div class="small text-muted mt-1">{{ __('Class teacher:') }}
                                                {{ $ward['class_teacher'] }}</div>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <span
                                            class="fw-semibold">{{ $attendancePercent !== null ? number_format($attendancePercent, 1) . '%' : __('No data') }}</span>
                                    </div>
                                    <div class="progress" style="height: 6px;">
                                        <div class="progress-bar bg-success" role="progressbar"
                                            style="width: {{ $attendancePercent !== null ? min($attendancePercent, 100) : 0 }}%">
                                        </div>
                                    </div>
                                    <div class="d-flex gap-3 mt-1 small text-muted">
                                        <span><i class="fas fa-check text-success me-1"></i>{{ $presentLabel }}
                                            {{ __('present/late') }}</span>
                                        <span><i
                                                class="fas fa-times text-danger me-1"></i>{{ $attendanceSummary['absent'] }}
                                            {{ __('absent') }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        ...
                                        @if ($latestGrade)
                                            @php
                                                $latestPercent =
                                                    $latestGrade->total_marks > 0
                                                        ? round(
                                                            ($latestGrade->marks_obtained / $latestGrade->total_marks) *
                                                                100,
                                                            1,
                                                        )
                                                        : null;
                                            @endphp
                                            <span
                                                class="badge bg-primary">{{ $latestPercent !== null ? $latestPercent . '%' : __('N/A') }}</span>
                                        @endif
                                    </div>
                                    @if ($latestGrade)
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <div class="text-end">
                                                @if ($latestGrade->grade_letter)
                                                    <span
                                                        class="badge bg-success-subtle text-success">{{ $latestGrade->grade_letter }}</span>
                                                @endif
                                                <div class="small text-muted">
                                                    {{ optional($latestGrade->assessment_date)->format('M j, Y') }}</div>
                                            </div>
                                        </div>
                                    @else
                                        ...
                                        <ul class="list-unstyled small mb-0">
                                            @foreach ($fees as $fee)
                                                <li
                                                    ...
                                                    <span
                                                        class="badge bg-warning bg-opacity-25 text-warning">{{ format_money($fee['balance']) }}</span>
                                                </li>
                                            @endforeach
                                        </ul>
                                    @endif
                                    ...
                                    @foreach ($feesDue as $item)
                                        <tr>
                                            ...
                                            <td>
                                                @if ($item['due_date'])
                                                    <span
                                                        class="badge {{ optional($item['due_date'])->isPast() ? 'bg-danger' : 'bg-primary' }}">{{ $item['due_date']->format('M j, Y') }}</span>
                                                @else
                                        ...
                                        <div>
                                            ...
                                            @if ($event->location)
                                                <div class="text-muted small"><i
                                                        class="fas fa-location-dot me-1"></i>{{ $event->location }}</div>
                                            @endif
                                        </div>
                                        <span class="badge"
                                            style="background: {{ $event->priority_color }};">{{ ucfirst($event->priority ?? 'normal') }}</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                                ...
                                <div class="list-group-item px-0">
                                    <div class="d-flex justify-content-between align-items-start gap-3">
                                        <div>
                                            ...
                                            <div class="text-muted small">{{ $grade['subject'] }} •
                                                {{ $grade['assessment_label'] }}</div>
                                            <div class="text-muted small">
                                                {{ optional($grade['recorded_at'])->format('M j, Y') }}</div>
                                        </div>
                                        <div class="text-end">
                                            @if ($grade['grade_letter'])
                                                <span
                                                    class="badge bg-success bg-opacity-25 text-success">{{ $grade['grade_letter'] }}</span>
                                            @endif
                                            <div class="fw-bold mt-1">
                                                {{ $grade['percentage'] !== null ? number_format($grade['percentage'], 1) . '%' : __('N/A') }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="list-group-item px-0">
                                    <div class="d-flex justify-content-between align-items-start gap-3">
                                        <div>
                                            ...
                                            <div class="text-muted small">
                                                {{ \Illuminate\Support\Str::limit($message?->content ?? __('Message unavailable'), 120) }}
                                            </div>
                                        </div>
                                        <div class="text-end small text-muted">
                                            {{ $recipient->created_at->diffForHumans() }}</div>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="row g-3">
                    @foreach ($plans as $plan)
                        <div class="col-12 col-lg-4">
                            <div
                                class="border rounded-4 h-100 p-3 d-flex flex-column gap-2 @if ($plan->is_highlighted) border-success @endif">
                                ...
                                <div class="mt-auto pt-2">
                                    ...
                                    <a class="btn btn-outline-parent w-100"
                                        href="mailto:hello@skolariscloud.com?subject={{ $subject }}">
