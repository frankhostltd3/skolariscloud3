...
<div class="d-flex align-items-center justify-content-between mb-4">
  ...
  <span class="badge text-bg-primary-subtle text-primary-emphasis">{{ tenant('id') }}</span>
</div>
  ...
  <div class="col-12 col-md-3">
    <div class="card shadow-sm">
      <div class="card-body">
        ...
        <div class="display-6">{{ number_format($activeStudentsCount) }}</div>
  ...
  <div class="col-12 col-md-3">
    <div class="card shadow-sm">
      <div class="card-body">
        ...
        <div class="display-6">{{ number_format($staffCount) }}</div>
  ...
  <div class="col-12 col-md-3">
    <div class="card shadow-sm">
      <div class="card-body">
        ...
        <div class="display-6">{{ $attendancePercentage }}%</div>
        <small class="text-muted">
          ...
          @else
            <span class="text-danger"><i class="bi bi-exclamation-circle-fill"></i> {{ __('Needs attention') }}</span>
          @endif
        </small>
      </div>
    </div>
  ...
  <div class="col-12 col-md-3">
    <div class="card shadow-sm">
      <div class="card-body">
        ...
        <div class="display-6">{{ currency_symbol() }}{{ number_format($feesOutstanding, 0) }}</div>
  ...
  <div class="col-12 col-md-3">
    <div class="card shadow-sm">
      <div class="card-body">
        ...
        <div class="display-6">{{ number_format($totalClasses) }}</div>
  ...
  <div class="card-body">
    ...
    <div class="d-flex flex-wrap gap-2 small">
      @canany(['manage fees','view fees'])
        <a class="btn btn-outline-primary btn-sm" href="{{ route('tenant.modules.fees.index') }}">{{ __('Fees') }}</a>
      @endcanany
      @hasanyrole('Admin|Staff')
        <a class="btn btn-outline-secondary btn-sm" href="{{ route('tenant.modules.teachers.create') }}">{{ __('Add new teacher') }}</a>
        <a class="btn btn-outline-secondary btn-sm" href="{{ route('tenant.modules.students.create') }}">{{ __('Add new student') }}</a>
        <a class="btn btn-outline-secondary btn-sm" href="{{ route('tenant.modules.subjects.create') }}">{{ __('Add new subject') }}</a>
        <a class="btn btn-outline-secondary btn-sm" href="{{ route('tenant.modules.classes.create') }}">{{ __('Create a new class') }}</a>
        <a class="btn btn-outline-secondary btn-sm" href="{{ route('tenant.modules.class_streams.create') }}">{{ __('Create a new class stream') }}</a>
      @endhasanyrole
    </div>
  </div>
</div>

{{-- Weekly Attendance Chart (Optional) --}}
            ...
            <thead>
              <tr>
                ...
                @foreach($weeklyAttendance as $day)
                  <th class="text-center">{{ $day['day'] }}<br><small class="text-muted">{{ $day['date'] }}</small></th>
                @endforeach
              </tr>
            </thead>
            <tbody>
              <tr>
                ...
                @foreach($weeklyAttendance as $day)
                  <td class="text-center">
                    <span class="badge bg-{{ $day['percentage'] >= 90 ? 'success' : ($day['percentage'] >= 75 ? 'warning' : 'danger') }}">
                      {{ $day['percentage'] }}%
                    </span>
                    <br>
                    <small class="text-muted">{{ $day['present'] }}/{{ $day['total'] }}</small>
                  </td>
                @endforeach
              </tr>
            </tbody>
...
<div class="row mb-4">
      ...
      <div class="card-body">
        <div class="list-group list-group-flush">
          ...
          <div class="list-group-item d-flex justify-content-between align-items-center px-0">
            ...
            <span class="badge bg-primary rounded-pill">{{ $class->active_students_count ?? 0 }}</span>
          </div>
          @endforeach
        </div>
      </div>
  ...
  <div class="col-12 col-lg-6">
    <!-- Bookstore Widget -->
    @include('tenant.components.bookstore.admin-widget')
  </div>
</div>
@else
<!-- Bookstore Widget (Full Width if no class data) -->
@include('tenant.components.bookstore.admin-widget')
@endif
