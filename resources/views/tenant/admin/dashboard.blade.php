...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        @if ($school = app('currentSchool'))
            <span class="badge text-bg-primary-subtle text-primary-emphasis">{{ $school->subdomain }}</span>
        @endif
    </div>
        ...
        <div class="col-12 col-md-3">
            <div class="card shadow-sm">
                <div class="card-body">
                    ...
                    <div class="display-6">{{ formatMoney($feesOutstanding) }}</div>
            ...
            <div class="d-flex flex-wrap gap-2 small">
                @canany(['manage fees', 'view fees'])
                    <a class="btn btn-outline-primary btn-sm"
                        href="{{ route('tenant.modules.fees.index') }}">{{ __('Fees') }}</a>
                @endcanany
                @hasanyrole('Admin|Staff')
                    <a class="btn btn-outline-secondary btn-sm"
                        href="{{ route('tenant.modules.teachers.create') }}">{{ __('Add new teacher') }}</a>
                    <a class="btn btn-outline-secondary btn-sm"
    ...
    @if (count($studentsByClass ?? []) > 0)
        <div class="row mb-4">
            ...
            <div class="col-12 col-lg-6">
                {{-- Bookstore Widget (Disabled - library_books table not yet implemented) --}}
