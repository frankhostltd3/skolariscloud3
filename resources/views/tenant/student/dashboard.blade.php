                            ...
                            <div class="p-3 bg-light rounded h-100">
                                ...
                                @if($upcoming['next_class'])
                                    <div class="d-flex justify-content-between">
                                        <div>
                                            <div class="fw-semibold">{{ $upcoming['next_class']->subject->name ?? 'Subject' }}</div>
                                            <small class="text-muted">{{ $upcoming['next_class']->class->name ?? 'Class' }} @if($upcoming['next_class']->stream) • {{ $upcoming['next_class']->stream->name }} @endif</small>
                                        </div>
                                        <div class="text-end">
                                            <span class="badge bg-primary">{{ \Illuminate\Support\Str::of($upcoming['next_class']->starts_at)->substr(0,5) }} - {{ \Illuminate\Support\Str::of($upcoming['next_class']->ends_at)->substr(0,5) }}</span>
                                        </div>
                                    </div>
                                @else
                                ...
                                <div class="mt-3 text-end">
                                    <a href="{{ route('tenant.student.schedule.index') }}" class="btn btn-sm btn-outline-primary">View timetable</a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                                ...
                                @forelse($upcoming['assignments'] as $a)
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <div>
                                            ...
                                            <small class="text-muted">{{ $a->subject->name ?? 'Subject' }} • Due {{ optional($a->due_date)->format('M j, g:i A') }}</small>
                                        </div>
                                        <a href="{{ route('tenant.student.classroom.exercises.index') }}" class="btn btn-sm btn-outline-primary">Open</a>
                                    </div>
                                @empty
                                    <div class="text-muted">No pending assignments</div>
                                @endforelse
                                <div class="mt-3 text-end">
                                    <a href="{{ route('tenant.student.classroom.exercises.index') }}" class="btn btn-sm btn-outline-primary">View all assignments</a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                                ...
                                @forelse($upcoming['quizzes'] as $q)
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <div>
                                            ...
                                            <small class="text-muted">@if($from) Opens {{ $from->format('M j, g:i A') }} @else Available @endif</small>
                                        </div>
                                        <a href="{{ route('tenant.student.quizzes.index') }}" class="btn btn-sm btn-outline-primary">View</a>
                                    </div>
                                @empty
                                    <div class="text-muted">No upcoming quizzes</div>
                                @endforelse
                                <div class="mt-3 text-end">
                                    <a href="{{ route('tenant.student.quizzes.index') }}" class="btn btn-sm btn-outline-primary">View all quizzes</a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                                ...
                                @forelse($upcoming['exams'] as $e)
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <div>
                                            ...
                                            <small class="text-muted">@if($e->start_time) Starts {{ $e->start_time->format('M j, g:i A') }} @else {{ optional($e->exam_date)->format('M j') }} @endif</small>
                                        </div>
                                        <a href="{{ route('tenant.student.quizzes.index') }}" class="btn btn-sm btn-outline-primary">Details</a>
                                    </div>
                                @empty
                                    <div class="text-muted">No scheduled exams</div>
                                @endforelse
                                <div class="mt-3 text-end">
                                    <a href="{{ route('tenant.student.quizzes.index') }}" class="btn btn-sm btn-outline-primary">View all exams</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        @if($continue['material'])
                            <div class="small">
                                ...
                                <div class="text-muted">{{ $continue['material']->subject->name ?? 'Subject' }} • {{ $continue['material']->class->name ?? 'Class' }}</div>
                                <a href="{{ route('tenant.student.classroom.materials.show', $continue['material']->id) }}" class="btn btn-sm btn-outline-primary mt-2">Resume</a>
                            </div>
                        @else
                    ...
                    <div>
                        ...
                        @if($continue['quiz_attempt'])
                            <div class="small">
                                ...
                                <a href="{{ route('tenant.student.quizzes.take', $continue['quiz_attempt']->quiz) }}" class="btn btn-sm btn-outline-primary mt-2">Resume</a>
                            </div>
                        @else
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <div class="d-flex align-items-center p-3 bg-light rounded">
                                <div class="me-3">
                                    <i class="fas fa-book-open fa-2x" style="color: var(--student-primary);"></i>
                                </div>
                                <div class="flex-grow-1">
                                    <h6 class="mb-1 fw-bold">{{ $subject->name }}</h6>
                                    <small class="text-muted">{{ $subject->code }}</small>
                                    <div class="small">
                                        <span class="badge bg-primary">{{ $subject->credit_hours ?? 3 }} Credits</span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="text-center mt-3">
                        <a href="{{ route('tenant.student.academic') }}" class="btn btn-outline-primary btn-sm">View All Grades</a>
                    </div>
                    @else
            ...
            <div class="card h-100">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <p class="text-muted mb-2">{{ $enrollment->classroom->description ?? 'Class description not available' }}</p>

                        <div class="row text-center">
                            <div class="col-6">
                                <div class="border-end">
                                    <h4 class="mb-0 text-primary">{{ $enrollment->classroom->capacity ?? 'N/A' }}</h4>
                                    <small class="text-muted">Capacity</small>
                                </div>
                            </div>
                            <div class="col-6">
                                <h4 class="mb-0 text-success">{{ $enrollment->classroom->enrollments->count() }}</h4>
                        ...
                        <div class="mb-2">
                            ...
                            <div class="progress progress-sm">
                                <div class="progress-bar bg-{{ $config['color'] }}"
                                    style="width: {{ array_sum($gradeDistribution) > 0 ? (($gradeDistribution[$letter] ?? 0) / array_sum($gradeDistribution)) * 100 : 0 }}%"></div>
                            </div>
                        </div>
                        @endforeach
                    </div>
