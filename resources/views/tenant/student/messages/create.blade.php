        ...
        <div class="col-lg-8">
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body p-4">
                    <form action="{{ route('tenant.student.messages.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf

                        <div class="mb-3">
                            <label for="recipient_id" class="form-label">{{ __('To (Recipient)') }} <span class="text-danger">*</span></label>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="subject" id="subject" class="form-control @error('subject') is-invalid @enderror" placeholder="{{ __('Enter message subject...') }}" value="{{ old('subject') }}" maxlength="255" required>
                            @error('subject')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="content" id="content" class="form-control @error('content') is-invalid @enderror" rows="10" placeholder="{{ __('Type your message here...') }}" required>{{ old('content') }}</textarea>
                            @error('content')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" name="attachment" id="attachment" class="form-control @error('attachment') is-invalid @enderror" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.gif">
                            <small class="text-muted">{{ __('Maximum file size: 10MB. Allowed formats: PDF, DOC, DOCX, JPG, PNG, GIF') }}</small>
                            @error('attachment')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
