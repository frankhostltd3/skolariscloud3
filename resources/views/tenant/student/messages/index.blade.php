                ...
                <form method="GET" action="{{ route('tenant.student.messages.index') }}" class="row g-2">
                    <div class="col-12 col-md-8">
                        <input type="text" name="search" class="form-control form-control-sm"
                            placeholder="{{ __('Search messages...') }}" value="{{ request('search') }}">
                    </div>
                ...
                <div class="card-body p-0">
                    <div class="list-group list-group-flush">
                        @foreach ($threads as $thread)
                            ...
                            @endphp
                            <a href="{{ route('tenant.student.messages.show', $thread->id) }}"
                                class="list-group-item list-group-item-action message-item {{ $isUnread ? 'unread' : '' }}">
                                    ...
                                    <div class="flex-grow-1">
                                        ...
                                        <small class="text-muted">
                                            ...
                                            @if ($thread->messages->count() > 1)
                                                <span class="ms-2"><i
                                                        class="bi bi-chat-dots me-1"></i>{{ $thread->messages->count() }}
                                                    {{ __('messages') }}</span>
                                            @endif
                                        </small>
                                    </div>
                                    <div class="ms-3">
                                        <form action="{{ route('tenant.student.messages.destroy', $thread->id) }}"
        ...
        @else
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center py-5">
                    ...
                    <p class="text-muted">{{ __('You have no messages yet. Start a conversation with your teachers!') }}
                    </p>
