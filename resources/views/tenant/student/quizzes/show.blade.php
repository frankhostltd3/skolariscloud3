                    ...
                    <div class="card-header bg-primary text-white">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                <a href="{{ route('tenant.student.quizzes.print', $quiz) }}" target="_blank"
                                    class="btn btn-sm btn-light text-primary">
                                    <i class="bi bi-printer me-1"></i>{{ __('Print') }}
                                </a>
                                <a href="{{ route('tenant.student.quizzes.download-pdf', $quiz) }}"
                                    class="btn btn-sm btn-outline-light text-white">
                                    <i class="bi bi-file-earmark-pdf me-1"></i>{{ __('Download PDF') }}
                                </a>
                                @php
                                    $quizShareLink = route('tenant.student.quizzes.show', $quiz);
                                    $quizShareText = 'Check out this quiz: ' . $quiz->title;
                                @endphp
                                <div class="btn-group">
                                    <button type="button" class="btn btn-sm btn-outline-light text-white dropdown-toggle"
                                        data-bs-toggle="dropdown" aria-expanded="false">
                                        <i class="bi bi-share-fill me-1"></i>{{ __('Share') }}
                                    </button>
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a class="dropdown-item" target="_blank" rel="noopener"
                                                href="mailto:?subject={{ rawurlencode($quizShareText) }}&body={{ rawurlencode($quizShareText . ' ' . $quizShareLink) }}">
                                                <i class="bi bi-envelope me-2"></i>{{ __('Email') }}
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" target="_blank" rel="noopener"
                                                href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode($quizShareLink) }}">
                                                <i class="bi bi-facebook me-2"></i>{{ __('Facebook') }}
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" target="_blank" rel="noopener"
                                                href="https://twitter.com/intent/tweet?url={{ urlencode($quizShareLink) }}&text={{ urlencode($quizShareText) }}">
                                                <i class="bi bi-twitter-x me-2"></i>{{ __('X') }}
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" target="_blank" rel="noopener"
                                                href="https://api.whatsapp.com/send?text={{ urlencode($quizShareText . ' ' . $quizShareLink) }}">
                    ...
                    <div class="card-body">
                        ...
                        <div class="alert alert-info">
                            ...
                            <ul class="mb-0 small">
                                ...
                                @if ($quiz->duration_minutes)
                                    <li>{{ __('You have') }} <strong>{{ $quiz->duration_minutes }}
                                            {{ __('minutes') }}</strong> {{ __('to complete this quiz') }}</li>
                                    <li class="text-danger">
                                        <strong>{{ __('The timer cannot be paused once started') }}</strong>
                                    </li>
                                @endif
                                        ...
                                        @if ($attempt->score !== null)
                                            @php
                                                ...
                                                $pointsTotal = max(
                                                    1,
                                                    $quiz->total_points ??
                                                        ($quiz->total_marks ?? ($quiz->questions->sum('marks') ?? 1)),
                                                );
                                                $scorePercent =
                                                    $rawScore !== null
                                                        ? round(($rawScore / $pointsTotal) * 100, 1)
                                                        : null;
                                            @endphp
                                            <p class="mb-2">
                                            ...
                                            <p class="mb-0 text-muted">{{ $rawScore }}/{{ $pointsTotal }}</p>
                                            @if ($attempt->grade)
                                                <div class="mt-2">
                                                    <span class="badge bg-success fs-5">{{ __('Grade:') }}
                                                        {{ $attempt->grade }}</span>
                                                </div>
                                            @endif
                                        @else
                        ...
                        <div class="card shadow-sm border-warning mb-4">
                            ...
                            <div class="card-body">
                                ...
                                @php
                                    $now = now();
                                    $isOpen =
                                        (!$quiz->start_at || $now->gte($quiz->start_at)) &&
                                        (!$quiz->end_at || $now->lte($quiz->end_at));
                                @endphp
                                @if ($isOpen)
                ...
                @else
                    ...
                    <div class="card shadow-sm border-primary">
                        ...
                        <div class="card-body">
                            ...
                            @else
                                <p class="mb-3">
                                    {{ __('Click the button below to start the quiz. Make sure you are ready!') }}</p>
                    ...
                    <div class="card-body">
                        ...
                        <ul class="list-unstyled small">
                            ...
                            @if ($attempt && $attempt->score !== null)
                                @php
                                    $rawScoreSidebar = $attempt->score;
                                    $pointsTotalSidebar = max(
                                        1,
                                        $quiz->total_points ??
                                            ($quiz->total_marks ?? ($quiz->questions->sum('marks') ?? 1)),
                                    );
                                    $scorePercentSidebar = round(($rawScoreSidebar / $pointsTotalSidebar) * 100, 1);
                                @endphp
                                <li class="mb-2">
                                    <strong>{{ __('Your Score:') }}</strong>
                                    <span class="text-success">{{ $scorePercentSidebar }}%
                                        ({{ $rawScoreSidebar }}/{{ $pointsTotalSidebar }})</span>
                                </li>
                            @endif
                        </ul>
                    </div>
