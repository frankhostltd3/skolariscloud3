        ...
        <form id="quiz-form" method="POST" action="{{ route('tenant.student.quizzes.submit', $quiz->id) }}">
            ...
            @if ($quiz->questions->count() > 0)
                @foreach ($quiz->questions as $index => $question)
                    @php
                        $questionPoints = $question->pivot->points ?? ($question->marks ?? null);
                        $questionText = $question->question ?? ($question->question_text ?? '');
                        $questionMedia =
                            $question->question_image ?? ($question->image_url ?? ($question->media_url ?? null));
                        ...
                        $normalizedOptions = [];
                        foreach ($rawOptions as $key => $optionValue) {
                            $label =
                                is_string($key) && $key !== '' ? strtoupper($key) : chr(65 + count($normalizedOptions));
                            if (is_array($optionValue)) {
                                $displayText =
                                    $optionValue['text'] ?? ($optionValue['label'] ?? ($optionValue['value'] ?? ''));
                                $value = $optionValue['value'] ?? $label;
                            } else {
                                $displayText = $optionValue;
                                $value = is_string($key) && $key !== '' ? $key : $optionValue;
                            }
                        ...
                        if ($questionMedia) {
                            $questionMediaUrl = filter_var($questionMedia, FILTER_VALIDATE_URL)
                                ? $questionMedia
                                : Storage::url($questionMedia);
                        }
                    @endphp
                    <div class="card shadow-sm border-0 mb-4">
                        <div class="card-header bg-light">
                            <div class="d-flex justify-content-between align-items-center">
                                <h6 class="mb-0">
                                    {{ __('Question') }} {{ $index + 1 }} {{ __('of') }}
                                    {{ $quiz->questions->count() }}
                                </h6>
                                @if ($questionPoints)
                                    <span class="badge bg-primary">
                                        {{ $questionPoints }} {{ __('points') }}
                                    </span>
                                @endif
                            </div>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                @if (!empty($questionText))
                                    <p class="fw-bold mb-2">{!! nl2br(e($questionText)) !!}</p>
                                @endif
                                @if ($questionMediaUrl)
                                    <img src="{{ $questionMediaUrl }}" alt="Question Image" class="img-fluid rounded mb-3"
                            ...
                            @php
                                ...
                                $savedScalarAnswer = is_array($savedAnswer)
                                    ? $savedAnswer['value'] ?? (reset($savedAnswer) ?? '')
                                    : $savedAnswer;
                            @endphp
                                    ...
                                    @foreach ($normalizedOptions as $option)
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="radio"
                                                ...
                                                value="{{ $option['value'] }}"
                                                {{ (string) $savedScalarAnswer === (string) $option['value'] ? 'checked' : '' }}>
                                            <label class="form-check-label"
                                                for="q{{ $question->id }}_option{{ $loop->index }}">
                                                <span class="fw-semibold me-2">{{ $option['label'] }}.</span>
                                                {{ $option['text'] !== '' ? $option['text'] : __('Option :label', ['label' => $option['label']]) }}
                                            </label>
                                        </div>
                                    @endforeach
                                @else
                                    <div class="alert alert-info mb-0">
                                        {{ __('This question does not have answer choices configured yet.') }}
                                    </div>
                                @endif
                            @elseif($question->type === 'true_false')
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="answers[{{ $question->id }}]"
                                        ...
                                        {{ (string) $savedScalarAnswer === 'true' ? 'checked' : '' }}>
                                ...
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="answers[{{ $question->id }}]"
                                        ...
                                        {{ (string) $savedScalarAnswer === 'false' ? 'checked' : '' }}>
                            ...
                            @elseif($question->type === 'short_answer')
                                <input type="text" class="form-control" name="answers[{{ $question->id }}]"
                                    ...
                                    value="{{ old('answers.' . $question->id, $savedScalarAnswer) }}">
                            @elseif($question->type === 'essay')
                                <textarea class="form-control" name="answers[{{ $question->id }}]" rows="5"
                                    placeholder="{{ __('Type your answer here...') }}">{{ old('answers.' . $question->id, $savedScalarAnswer) }}</textarea>
                            @else
                                <div class="alert alert-warning">
        ...
        <div class="card shadow-sm border-0 mt-4">
            <div class="card-body">
                ...
                <p class="small text-muted mt-2 mb-0">
                    <span id="answered-count">0</span> {{ __('of') }} {{ $quiz->questions->count() }}
