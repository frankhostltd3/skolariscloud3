                    ...
                    <form action="{{ route('tenant.student.notes.personal.update', $personalNote->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   id="title" 
                                   name="title" 
                                   value="{{ old('title', $personalNote->title) }}" 
                                   required
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('content') is-invalid @enderror" 
                                      ...
                                      rows="15" 
                                      required
                                      placeholder="{{ __('Write your notes here...') }}">{{ old('content', $personalNote->content) }}</textarea>
                            @error('content')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">
                                <span class="me-3">
                                    ...
                                    {{ __('Word Count:') }} <strong id="wordCount">{{ $personalNote->word_count }}</strong>
                                </span>
                                <span>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="d-flex gap-2">
                                ...
                                <label class="btn btn-outline-primary" for="color1" style="width: 50px; height: 40px; background-color: #0d6efd;"></label>

                                <input type="radio" class="btn-check" name="color" id="color2" value="#198754"
                                       {{ old('color', $personalNote->color) == '#198754' ? 'checked' : '' }}>
                                <label class="btn btn-outline-success" for="color2" style="width: 50px; height: 40px; background-color: #198754;"></label>

                                <input type="radio" class="btn-check" name="color" id="color3" value="#ffc107"
                                       {{ old('color', $personalNote->color) == '#ffc107' ? 'checked' : '' }}>
                                <label class="btn btn-outline-warning" for="color3" style="width: 50px; height: 40px; background-color: #ffc107;"></label>

                                <input type="radio" class="btn-check" name="color" id="color4" value="#dc3545"
                                       {{ old('color', $personalNote->color) == '#dc3545' ? 'checked' : '' }}>
                                <label class="btn btn-outline-danger" for="color4" style="width: 50px; height: 40px; background-color: #dc3545;"></label>

                                <input type="radio" class="btn-check" name="color" id="color5" value="#6f42c1"
                                       {{ old('color', $personalNote->color) == '#6f42c1' ? 'checked' : '' }}>
                                <label class="btn btn-outline-secondary" for="color5" style="width: 50px; height: 40px; background-color: #6f42c1;"></label>

                                <input type="radio" class="btn-check" name="color" id="color6" value="#0dcaf0"
                                       {{ old('color', $personalNote->color) == '#0dcaf0' ? 'checked' : '' }}>
                                <label class="btn btn-outline-info" for="color6" style="width: 50px; height: 40px; background-color: #0dcaf0;"></label>
                            </div>
                        </div>
                ...
                <div class="card-body">
                    <div class="row small text-muted">
                        <div class="col-md-6">
                            ...
                            <strong>{{ __('Created:') }}</strong> {{ $personalNote->created_at->format('M d, Y H:i') }}
                        </div>
                        <div class="col-md-6">
                            <i class="bi bi-calendar-check me-1"></i>
                            <strong>{{ __('Updated:') }}</strong> {{ $personalNote->updated_at->format('M d, Y H:i') }}
                        </div>
                    </div>
                </div>
            </div>
