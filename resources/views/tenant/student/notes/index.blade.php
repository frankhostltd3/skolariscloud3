        ...
        @else
                    ...
                    <form method="GET" action="{{ route('tenant.student.notes.index') }}" class="row g-2">
                        <div class="col-12 col-md-4">
                            <input type="text" name="search" class="form-control form-control-sm"
                                placeholder="{{ __('Search notes and materials...') }}" value="{{ $search ?? '' }}">
                        </div>
                            ...
                            <select name="subject_id" class="form-select form-select-sm">
                                ...
                                @foreach ($subjects as $subject)
                                    <option value="{{ $subject->id }}"
                                        {{ ($subjectId ?? '') == $subject->id ? 'selected' : '' }}>{{ $subject->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-6 col-md-3">
                            <select name="view" class="form-select form-select-sm">
                                <option value="materials" {{ ($view ?? 'materials') == 'materials' ? 'selected' : '' }}>
                                    {{ __('Teacher Materials') }}</option>
                                <option value="personal" {{ ($view ?? '') == 'personal' ? 'selected' : '' }}>
                                    {{ __('My Personal Notes') }}</option>
                            </select>
                        </div>
            ...
            <ul class="nav nav-tabs mb-4" role="tablist">
                <li class="nav-item" role="presentation">
                    <a class="nav-link {{ ($view ?? 'materials') == 'materials' ? 'active' : '' }}"
                        href="{{ route('tenant.student.notes.index', array_merge(request()->query(), ['view' => 'materials'])) }}">
                ...
                <li class="nav-item" role="presentation">
                    <a class="nav-link {{ ($view ?? '') == 'personal' ? 'active' : '' }}"
                        href="{{ route('tenant.student.notes.index', array_merge(request()->query(), ['view' => 'personal'])) }}">
                                    ...
                                    <div class="card-body p-3">
                                        <div class="d-flex justify-content-between align-items-start mb-2">
                                            <div class="flex-grow-1">
                                                <span
                                                    class="badge bg-{{ $material->type == 'document' ? 'primary' : ($material->type == 'video' ? 'danger' : 'info') }} mb-2">
                                                    <i
                                                        class="bi bi-{{ $material->type == 'document' ? 'file-text' : ($material->type == 'video' ? 'play-circle' : 'link-45deg') }} me-1"></i>
                                                    {{ $material->type_label }}
                                                </span>
                                                <h6 class="mb-1">
                                                    <a href="{{ route('tenant.student.notes.show', $material->id) }}"
                                                        class="text-decoration-none text-dark">{{ $material->title }}</a>
                                                </h6>
                                            </div>
                                        </div>
                                        @if ($material->description)
                                            <p class="small text-muted mb-2">{{ Str::limit($material->description, 100) }}
                                            </p>
                                        @endif
                                        <div class="row g-2 small text-muted mb-2">
                                            <div class="col-6"><i
                                                    class="bi bi-book me-1"></i>{{ $material->subject->name ?? 'N/A' }}
                                            </div>
                                            <div class="col-6"><i
                                                    class="bi bi-person me-1"></i>{{ $material->teacher->name ?? 'N/A' }}
                                            </div>
                                            @if ($material->file_size)
                                                <div class="col-6"><i
                                                        class="bi bi-hdd me-1"></i>{{ $material->file_size_formatted }}
                                                </div>
                                            @endif
                                            <div class="col-6"><i
                                                    class="bi bi-eye me-1"></i>{{ $material->views_count }}
                                                {{ __('views') }}</div>
                                        </div>
                                        <div class="d-flex gap-1">
                                            ...
                                            @if ($material->is_downloadable && $material->file_path)
                                                <a href="{{ route('tenant.student.notes.download', $material->id) }}"
                ...
                @if ($personalNotes->count() > 0)
                    <div class="row">
                        @foreach ($personalNotes as $note)
                            <div class="col-12 col-md-6 col-lg-4 mb-3">
                                <div class="card h-100 border-0 shadow-sm hover-shadow"
                                    style="border-left: 4px solid {{ $note->color ?? '#0d6efd' }} !important;">
                                    <div class="card-body p-3">
                                        <div class="d-flex justify-content-between align-items-start mb-2">
                                            <div class="flex-grow-1">
                                                ...
                                                @if ($note->subject)
                                                    <span
                                                        class="badge bg-secondary small">{{ $note->subject->name }}</span>
                                                @endif
                                            </div>
                                            <button type="button" class="btn btn-sm btn-link p-0 favorite-btn"
                                                data-note-id="{{ $note->id }}"
                                                data-favorite="{{ $note->is_favorite ? 'true' : 'false' }}">
                                                <i class="bi bi-star{{ $note->is_favorite ? '-fill text-warning' : '' }}"
                                        ...
                                        <div class="small text-muted mb-2">
                                            ...
                                            <span class="ms-2"><i
                                                    class="bi bi-file-text me-1"></i>{{ $note->word_count }}
                                                {{ __('words') }}</span>
                                        </div>
                                        <div class="d-flex gap-1">
                                            <a href="{{ route('tenant.student.notes.personal.edit', $note->id) }}"
                                                class="btn btn-outline-primary btn-sm flex-grow-1">
                                                <i class="bi bi-pencil me-1"></i>{{ __('Edit') }}
                                            </a>
                                            <form action="{{ route('tenant.student.notes.personal.destroy', $note->id) }}"
