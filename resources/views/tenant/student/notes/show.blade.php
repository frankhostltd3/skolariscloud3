                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body p-4">
                        ...
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div>
                                <span
                                    class="badge bg-{{ $note->type == 'document' ? 'primary' : ($note->type == 'video' ? 'danger' : 'info') }} mb-2">
                                    <i
                                        class="bi bi-{{ $note->type == 'document' ? 'file-text' : ($note->type == 'video' ? 'play-circle' : 'link-45deg') }} me-1"></i>
                                    {{ $note->type_label }}
                                </span>
                        ...
                        @if ($note->type == 'youtube' && $note->youtube_embed_url)
                            <div class="ratio ratio-16x9 mb-4">
                                <iframe src="{{ $note->youtube_embed_url }}" allowfullscreen class="rounded"></iframe>
                            </div>
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-start">
                            @if ($note->is_downloadable && $note->file_path)
                                <a href="{{ route('tenant.student.notes.download', $note->id) }}" class="btn btn-success">
                                    <i class="bi bi-download me-2"></i>{{ __('Download') }}
                                </a>
                            @endif

                            @if ($note->external_url)
                                <a href="{{ $note->external_url }}" target="_blank" class="btn btn-primary">
                                    <i class="bi bi-box-arrow-up-right me-2"></i>{{ __('Open Link') }}
                                </a>
                            @endif

                            <a href="{{ route('tenant.student.notes.index') }}" class="btn btn-outline-secondary">
