                                ...
                                <p class="mb-3">
                                    @if ($assignment->due_date)
                                        ...
                                        @if ($assignment->due_date->isPast() && !$isPaid)
                                            <br><span
                                                class="badge bg-danger">{{ $assignment->due_date->diffInDays(now()) }} days
                                                overdue</span>
                                        @endif
                                    @else
                ...
                <div class="card border-0 shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        @if (!$isPaid)
                            <form method="POST" action="{{ route('tenant.student.fees.pay', $fee) }}">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="amount"
                                        class="form-control @error('amount') is-invalid @enderror" min="0.01"
                                        step="0.01" max="{{ max($outstanding, 0) }}"
                                        value="{{ old('amount', $outstanding) }}" required>
                                    <div class="form-text">
                                        {{ __('Enter the amount you wish to pay (partial payments allowed).') }}</div>
                                    @error('amount')
                            ...
                            <hr>
                            <a href="{{ route('tenant.student.fees.bank_slip', $fee) }}"
                                class="btn btn-outline-secondary w-100">
                                <i class="bi bi-download me-2"></i>{{ __('Download Bank Payment Slip (PDF)') }}
                            </a>
                            <div class="mt-3">
                                <form method="POST" action="{{ route('tenant.student.fees.upload_proof', $fee) }}"
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <input type="text" name="reference"
                                            ...
                                            placeholder="e.g. Deposit Slip No or Narrative">
                                        @error('reference')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
