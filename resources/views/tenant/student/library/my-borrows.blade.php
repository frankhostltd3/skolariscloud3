                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="mb-0 {{ $statistics['overdue'] > 0 ? 'text-danger' : '' }}">{{ $statistics['overdue'] }}</h3>
                        </div>
                        <div class="{{ $statistics['overdue'] > 0 ? 'text-danger' : 'text-secondary' }}" style="font-size: 2rem;">
            ...
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="{{ $statistics['total_fines'] > 0 ? 'text-warning' : 'text-secondary' }}" style="font-size: 2rem;">
            ...
            <form method="GET" action="{{ route('tenant.student.library.my-borrows') }}" class="row g-3">
                <div class="col-md-3">
                    ...
                    <select name="status" class="form-select" onchange="this.form.submit()">
                        <option value="active" {{ $status == 'active' ? 'selected' : '' }}>{{ __('Active Borrows') }}</option>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($transaction->status === 'borrowed' && !$transaction->isOverdue())
                                            <br><small class="text-muted">{{ $transaction->due_date->diffInDays(now()) }} days left</small>
                                        @endif
                                    </td>
                                    <td>
                                        @if($transaction->status === 'returned')
                                            ...
                                            @if($transaction->returned_at)
                                                <br><small class="text-muted">{{ $transaction->returned_at->format('M d, Y') }}</small>
                                            @endif
                                        @elseif($transaction->isOverdue())
                                            <span class="badge bg-danger">
                                                <i class="bi bi-exclamation-triangle me-1"></i>{{ __('Overdue') }}
                                            </span>
                                            <br><small class="text-danger">{{ $transaction->days_overdue }} days late</small>
                                        @else
