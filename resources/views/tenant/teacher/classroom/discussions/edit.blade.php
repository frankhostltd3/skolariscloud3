...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1 class="h3 mb-0">
                    <i class="bi bi-pencil-square me-2 text-primary"></i>Edit Discussion
                </h1>
                <p class="text-muted mb-0">Update discussion details</p>
            </div>
            <div>
                <a href="{{ route('tenant.teacher.classroom.discussions.show', $discussion) }}"
        ...
        <form action="{{ route('tenant.teacher.classroom.discussions.update', $discussion) }}" method="POST"
                ...
                <div class="col-lg-8">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <!-- Title -->
                            <div class="mb-3">
                                <label for="title" class="form-label">Discussion Title <span
                                        class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('title') is-invalid @enderror"
                                    id="title" name="title" value="{{ old('title', $discussion->title) }}"
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content" rows="8"
                                    placeholder="Write your discussion post..." required>{{ old('content', $discussion->content) }}</textarea>
                                @error('content')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                    ...
                                    <select class="form-select @error('class_id') is-invalid @enderror" id="class_id"
                                        ...
                                        @foreach ($classes as $class)
                                            <option value="{{ $class->id }}"
                                                {{ old('class_id', $discussion->class_id) == $class->id ? 'selected' : '' }}>
                                                {{ $class->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-select @error('subject_id') is-invalid @enderror" id="subject_id"
                                        ...
                                        @foreach ($subjects as $subject)
                                            <option value="{{ $subject->id }}"
                                                {{ old('subject_id', $discussion->subject_id) == $subject->id ? 'selected' : '' }}>
                                                {{ $subject->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <!-- Type -->
                            <div class="mb-3">
                                <label for="type" class="form-label">Discussion Type <span
                                        class="text-danger">*</span></label>
                                <select class="form-select @error('type') is-invalid @enderror" id="type"
                                    name="type" required>
                                    <option value="general"
                                        {{ old('type', $discussion->type) == 'general' ? 'selected' : '' }}>
                                        💬 General Discussion
                                    </option>
                                    <option value="question"
                                        {{ old('type', $discussion->type) == 'question' ? 'selected' : '' }}>
                                        ❓ Question
                                    </option>
                                    <option value="announcement"
                                        {{ old('type', $discussion->type) == 'announcement' ? 'selected' : '' }}>
                                        📢 Announcement
                                    </option>
                                    <option value="poll"
                                        {{ old('type', $discussion->type) == 'poll' ? 'selected' : '' }}>
                                        📊 Poll
                                    </option>
                                </select>
                                @error('type')
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_pinned" name="is_pinned"
                                        value="1" {{ old('is_pinned', $discussion->is_pinned) ? 'checked' : '' }}>
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="allow_replies"
                                        name="allow_replies" value="1"
                                        {{ old('allow_replies', $discussion->allow_replies) ? 'checked' : '' }}>
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="requires_approval"
                                        ...
                                        {{ old('requires_approval', $discussion->requires_approval) ? 'checked' : '' }}>
                            ...
                            <div class="mb-0">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_locked" name="is_locked"
                                        value="1" {{ old('is_locked', $discussion->is_locked) ? 'checked' : '' }}>
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <button type="submit" class="btn btn-primary">
                                    <i class="bi bi-check-circle me-2"></i>Update Discussion
                                </button>
                            </div>
                        </div>
                    </div>
