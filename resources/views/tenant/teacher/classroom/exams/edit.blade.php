...
@section('title', 'Edit Online Exam')
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1 class="h3 mb-0">
                    <i class="bi bi-pencil-square me-2 text-primary"></i>Edit Online Exam
                </h1>
                <p class="text-muted mb-0">Update exam details and settings</p>
            </div>
            <div>
        ...
        <form action="{{ route('tenant.teacher.classroom.exams.update', $exam->id) }}" method="POST" id="examForm">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('instructions') is-invalid @enderror" id="instructions" name="instructions"
                                    rows="4" placeholder="Exam rules and instructions">{{ old('instructions', $exam->instructions) }}</textarea>
                                @error('instructions')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="creation_method" class="form-label">Creation Method <span
                                            class="text-danger">*</span></label>
                                    <select class="form-select @error('creation_method') is-invalid @enderror"
                                        id="creation_method" name="creation_method" required>
                                        <option value="manual"
                                            {{ old('creation_method', $exam->creation_method ?? 'manual') === 'manual' ? 'selected' : '' }}>
                                            Manual authoring</option>
                                        <option value="automatic"
                                            {{ old('creation_method', $exam->creation_method) === 'automatic' ? 'selected' : '' }}>
                                            Automatic template</option>
                                        <option value="ai"
                                            {{ old('creation_method', $exam->creation_method) === 'ai' ? 'selected' : '' }}>
                                            AI assisted</option>
                                    </select>
                                    <small class="text-muted">Automatic or AI modes will reference generation notes and
                                        recent syllabus mappings.</small>
                                ...
                                <div class="col-md-6 mb-3">
                                    <label for="activation_mode" class="form-label">Activation Mode <span
                                            class="text-danger">*</span></label>
                                    <select class="form-select @error('activation_mode') is-invalid @enderror"
                                        id="activation_mode" name="activation_mode" required>
                                        <option value="schedule"
                                            {{ old('activation_mode', $exam->activation_mode ?? 'schedule') === 'schedule' ? 'selected' : '' }}>
                                            Scheduled window</option>
                                        <option value="manual"
                                            {{ old('activation_mode', $exam->activation_mode) === 'manual' ? 'selected' : '' }}>
                                            Manual activation</option>
                                        <option value="auto"
                                            {{ old('activation_mode', $exam->activation_mode) === 'auto' ? 'selected' : '' }}>
                                            Auto start with class</option>
                                    </select>
                                    <small class="text-muted">Scheduled mode auto-activates after approval; manual leaves
                                        control with you.</small>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    <label for="starts_at" class="form-label">Start Date & Time <span
                                            class="text-danger">*</span></label>
                                    <input type="datetime-local"
                                        class="form-control @error('starts_at') is-invalid @enderror" id="starts_at"
                                        name="starts_at"
                                        value="{{ old('starts_at', $exam->starts_at ? \Carbon\Carbon::parse($exam->starts_at)->format('Y-m-d\TH:i') : '') }}"
                                        required>
                                    @error('starts_at')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-md-4 mb-3">
                                    <label for="ends_at" class="form-label">End Date & Time <span
                                            class="text-danger">*</span></label>
                                    <input type="datetime-local"
                                        class="form-control @error('ends_at') is-invalid @enderror" id="ends_at"
                                        name="ends_at"
                                        value="{{ old('ends_at', $exam->ends_at ? \Carbon\Carbon::parse($exam->ends_at)->format('Y-m-d\TH:i') : '') }}"
                                        required>
                                    @error('ends_at')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-4 mb-3">
                                    <label for="grading_method" class="form-label">Grading Method <span
                                            class="text-danger">*</span></label>
                                    <select class="form-select @error('grading_method') is-invalid @enderror"
                                        id="grading_method" name="grading_method" required>
                                        <option value="auto"
                                            {{ old('grading_method', $exam->grading_method) == 'auto' ? 'selected' : '' }}>
                                            Auto Grading
                                        </option>
                                        <option value="manual"
                                            {{ old('grading_method', $exam->grading_method) == 'manual' ? 'selected' : '' }}>
                                            Manual Grading
                                        </option>
                                        <option value="mixed"
                                            {{ old('grading_method', $exam->grading_method) == 'mixed' ? 'selected' : '' }}>
                                            Mixed (Auto + Manual)
                                        </option>
                                    </select>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="allow_backtrack"
                                            name="allow_backtrack" value="1"
                                            {{ old('allow_backtrack', $exam->allow_backtrack) ? 'checked' : '' }}>
                                ...
                                <div class="col-md-6 mb-3">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="disable_copy_paste"
                                            name="disable_copy_paste" value="1"
                                            {{ old('disable_copy_paste', $exam->disable_copy_paste) ? 'checked' : '' }}>
                                ...
                                <div class="col-md-6 mb-3">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="proctored" name="proctored"
                                            value="1" {{ old('proctored', $exam->proctored) ? 'checked' : '' }}>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number"
                                        class="form-control @error('max_tab_switches') is-invalid @enderror"
                                        id="max_tab_switches" name="max_tab_switches"
                                        value="{{ old('max_tab_switches', $exam->max_tab_switches) }}" min="0"
                                        max="20">
                                ...
                                <div class="col-md-6 mb-3">
                                    <label for="auto_submit_on" class="form-label">Auto Submit <span
                                            class="text-danger">*</span></label>
                                    <select class="form-select @error('auto_submit_on') is-invalid @enderror"
                                        id="auto_submit_on" name="auto_submit_on" required>
                                        <option value="time_up"
                                            {{ old('auto_submit_on', $exam->auto_submit_on) == 'time_up' ? 'selected' : '' }}>
                                            When Time is Up
                                        </option>
                                        <option value="manual"
                                            {{ old('auto_submit_on', $exam->auto_submit_on) == 'manual' ? 'selected' : '' }}>
                                            Manual Only
                                        </option>
                                        <option value="both"
                                            {{ old('auto_submit_on', $exam->auto_submit_on) == 'both' ? 'selected' : '' }}>
                                            Both
                                        </option>
                                    </select>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            @php($statusLocked = in_array($exam->status, ['active', 'completed', 'archived']))
                            <div class="mb-3">
                                ...
                                @if ($statusLocked)
                                    <div class="form-control bg-light">
                                        {{ ucfirst($exam->status) }} <span
                                            class="text-muted">({{ __('managed by admin') }})</span>
                                    </div>
                            ...
                            <div class="alert alert-info mb-0">
                                <small>
                                    ...
                                    <strong>Draft</strong>: {{ __('Keep editing privately') }}<br>
                                    <strong>Ready</strong>: {{ __('Submit for review; exam unlocks after approval') }}<br>
                                    @if ($statusLocked)
                                        <span
                                            class="text-muted">{{ __('Active/completed exams are locked by the admin team.') }}</span>
                                    @endif
                                </small>
                            </div>
                        </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <a href="{{ route('tenant.teacher.classroom.exams.index') }}"
                                    class="btn btn-outline-secondary">
                                    <i class="bi bi-x-circle me-2"></i>Cancel
                                </a>
                                <div>
                                    <button type="submit" class="btn btn-primary">
                                        <i class="bi bi-save me-2"></i>Update Exam
                                    </button>
                                </div>
                            </div>
                        </div>
