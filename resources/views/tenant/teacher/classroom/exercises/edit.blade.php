                        ...
                        <form action="{{ route('tenant.teacher.classroom.exercises.update', $exercise) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control wysiwyg-editor @error('description') is-invalid @enderror" id="description"
                                    name="description" data-placeholder="Describe the assignment context">{!! old('description', $exercise->description) !!}</textarea>
                                @error('description')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control wysiwyg-editor @error('instructions') is-invalid @enderror" id="instructions"
                                    name="instructions" data-placeholder="Provide step-by-step instructions">{!! old('instructions', $exercise->instructions) !!}</textarea>
                                @error('instructions')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                                <div class="form-text">These instructions will be shown to students when they view the
                                    assignment.</div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control wysiwyg-editor wysiwyg-large @error('content') is-invalid @enderror" id="content"
                                    name="content" data-placeholder="Enter the assignment content here">{!! old('content', $exercise->content) !!}</textarea>
                                @error('content')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                                <div class="form-text">
                                    ...
                                    students need to complete. Use Heading 2/3 in the editor to label each question so the
                                    outline card is generated for you and for the students.
                                </div>
                            </div>
                                    ...
                                    <select class="form-select @error('class_id') is-invalid @enderror" id="class_id"
                                        ...
                                        @foreach ($classes as $class)
                                            <option value="{{ $class->id }}"
                                                {{ old('class_id', $exercise->class_id) == $class->id ? 'selected' : '' }}>
                                                {{ $class->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-select @error('subject_id') is-invalid @enderror" id="subject_id"
                                        ...
                                        @foreach ($subjects as $subject)
                                            <option value="{{ $subject->id }}"
                                                {{ old('subject_id', $exercise->subject_id) == $subject->id ? 'selected' : '' }}>
                                                {{ $subject->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" class="form-control @error('max_score') is-invalid @enderror"
                                        ...
                                        value="{{ old('max_score', $exercise->max_score) }}" min="0" step="0.01"
                                        required>
                                    @error('max_score')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('submission_type') is-invalid @enderror"
                                    ...
                                    <option value="file"
                                        {{ old('submission_type', $exercise->submission_type) == 'file' ? 'selected' : '' }}>
                                        File Upload Only
                                    </option>
                                    <option value="text"
                                        {{ old('submission_type', $exercise->submission_type) == 'text' ? 'selected' : '' }}>
                                        Text Answer Only
                                    </option>
                                    <option value="both"
                                        {{ old('submission_type', $exercise->submission_type) == 'both' ? 'selected' : '' }}>
                                        Both File and Text
                                    </option>
                                </select>
                            ...
                            <div class="card bg-light mb-3">
                                <div class="card-body">
                                    ...
                                    <div id="penalty-section"
                                        class="{{ old('allow_late_submission', $exercise->allow_late_submission) ? '' : 'd-none' }}">
                                        ...
                                        <div class="form-text">
                                            ...
                                            Example: 10% penalty means if a student scores 80/100, they get 72/100 for a
                                            late submission.
                                        </div>
                                    </div>
                                </div>
                            ...
                            @if ($exercise->submissions()->count() > 0)
                                <div class="alert alert-warning" role="alert">
                                    ...
                                    <strong>Note:</strong> This assignment has {{ $exercise->submissions()->count() }}
                                    submission(s).
