...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex align-items-center gap-3">
            @if ($teacherAvatar)
                <img src="{{ $teacherAvatar }}" alt="{{ auth()->user()->name }}" class="rounded-circle border" width="64"
    ...
    @if (!empty($registrationInsights))
        <x-registration.pipeline context="teacher" :stages="$registrationInsights['stages']" :summary="$registrationInsights['summary']" :mode="$registrationInsights['mode'] ?? null"
            :next-action="$registrationInsights['next_action'] ?? null" />

        @php
            $recentAdmissions = collect($registrationInsights['recent'] ?? []);
        @endphp
        @if ($recentAdmissions->isNotEmpty())
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center">
                    <h5 class="mb-0"><i class="bi bi-people me-2"></i>{{ __('Newest learners in your classes') }}</h5>
                    <span class="badge bg-primary-subtle text-primary">{{ __('Admissions update') }}</span>
                </div>
                    ...
                    <table class="table table-sm align-middle mb-0">
                        ...
                        <tbody>
                            @foreach ($recentAdmissions as $student)
                                @php
                                    $status = strtolower($student->approval_status ?? 'pending');
                                    $badges = [
                                ...
                                <tr>
                                    <td>
                                        <div class="fw-semibold">{{ $student->name ?? __('Student') }}</div>
                                        <div class="text-muted small">{{ $student->email }}</div>
                                    </td>
                                    <td><span class="badge {{ $badgeClass }}">{{ ucfirst($status) }}</span></td>
                                    <td class="text-muted small">{{ optional($student->created_at)->diffForHumans() }}</td>
                                    <td>
                            ...
                            @foreach ($allClasses as $class)
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    <span>
                                        ...
                                        @endif
                                        <span class="text-muted small ms-2">Year:
                                            {{ $class->academicYear->name ?? '-' }}</span>
                                    </span>
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    <span>
                                        ...
                                        @if ($subject->pivot && $subject->pivot->class_id)
                                            <small
                                                class="text-muted">({{ $subject->pivot->class->name ?? 'Class' }})</small>
                                        @endif
                                    </span>
                                ...
                                <div class="card border">
                                    <div class="card-header bg-light">
                                        <h6 class="mb-0">
                                            ...
                                            @endif
                                            <span class="badge bg-secondary float-end">{{ $class->students->count() }}
                                                students</span>
                                        </h6>
                                    </div>
                                    <div class="card-body p-2">
                                        <div class="row g-1">
                                            @foreach ($class->students->take(6) as $student)
                                                <div class="col-6">
                                                    <small class="text-truncate d-block">
                                                        <i class="bi bi-person-circle me-1"></i>{{ $student->first_name }}
                                                        {{ $student->last_name }}
                                                    </small>
                                                </div>
                                            @endforeach
                                            @if ($class->students->count() > 6)
                                                <div class="col-12">
                                                    <small class="text-muted">... and {{ $class->students->count() - 6 }}
                                                        more</small>
                                                </div>
                                            @endif
                                        </div>
