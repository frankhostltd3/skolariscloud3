        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <div class="d-flex gap-2">
                ...
                @if ($filters['class_id'] ?? false)
                    <a href="{{ route('tenant.academics.timetable.class', $filters['class_id']) }}"
                        class="btn btn-outline-primary">Class View</a>
                @endif
            </div>
        </div>
        ...
        <form method="get" class="row g-2 mb-3">
            ...
            <div class="col-md-3">
                <select name="day_of_week" class="form-select" onchange="this.form.submit()">
                    ...
                    @for ($d = 1; $d <= 7; $d++)
                        <option value="{{ $d }}" @selected(($filters['day_of_week'] ?? '') == $d)>
            ...
            <div class="col-md-3 text-end">
                ...
                <a class="btn btn-success ms-2" href="{{ route('tenant.academics.timetable.generate') }}">
                ...
                <tbody>
                    @forelse($entries as $e)
                        <tr>
                            ...
                            @endcan
                            <td>{{ \Carbon\Carbon::create()->startOfWeek()->addDays($e->day_of_week - 1)->format('l') }}
                            </td>
                            ...
                            <td class="text-end">
                                @can('manage timetable')
                                    <a href="{{ route('tenant.academics.timetable.edit', $e) }}"
                                        class="btn btn-sm btn-outline-primary">Edit</a>
                                    <a href="{{ route('tenant.academics.timetable.class', $e->class_id) }}"
                                        class="btn btn-sm btn-outline-secondary">Class</a>
                                    @if ($e->class_stream_id)
                                        <a href="{{ route('tenant.academics.timetable.stream', $e->class_stream_id) }}"
                                            class="btn btn-sm btn-outline-secondary">Stream</a>
                                    @endif
                                    @if ($e->teacher_id)
                                        <a href="{{ route('tenant.academics.timetable.teacher', $e->teacher_id) }}"
                                            class="btn btn-sm btn-outline-secondary">Teacher</a>
                                    @endif
                                    <form action="{{ route('tenant.academics.timetable.destroy', $e) }}" method="post"
