...
@section('content')
    <div class="container py-3">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div>
                <h1 class="h4 mb-1">{{ __('Teacher Timetable') }} - {{ $teacher->full_name ?? $teacher->name }}</h1>
                <p class="text-muted mb-0">{{ __('Weekly teaching schedule') }}</p>
            </div>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <small class="text-muted">{{ __('Teacher assignments grouped by day') }}</small>
            </div>
            <div class="card-body">
                <div class="row g-3">
                                        ...
                                        @foreach ($entries as $e)
                                            <li class="list-group-item small">
                                                ...
                                                <div class="d-flex justify-content-between mt-1">
                                                    <span class="text-muted">{{ $e->class->name ?? '—' }} @if ($e->stream)
                                                            / {{ $e->stream->name }}
                                                        @endif
                                                    </span>
                ...
                @if (collect($schedule)->flatten()->count() === 0)
                    <p class="text-muted text-center my-4">{{ __('No timetable entries found for this teacher.') }}</p>
                @endif
            </div>
        </div>
    </div>
