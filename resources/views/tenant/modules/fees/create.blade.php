...
<div class="d-flex align-items-center justify-content-between mb-4">
  <div>
    <h1 class="h4 fw-semibold mb-0">{{ __('Create New Fee') }}</h1>
    <div class="small text-secondary">{{ __('Add a new fee structure for students') }}</div>
  </div>
  <a href="{{ route('tenant.modules.fees.index') }}" class="btn btn-outline-secondary">
    <i class="bi bi-arrow-left me-1"></i>{{ __('Back to Fees') }}
  </a>
</div>
...
<div class="row">
  <div class="col-lg-8 mx-auto">
    <div class="card shadow-sm">
      <div class="card-body">
        <form method="POST" action="{{ route('tenant.modules.fees.store') }}">
          @csrf
          ...
          <div class="row">
            ...
            <div class="col-md-6 mb-3">
              ...
              <select class="form-select @error('category') is-invalid @enderror" id="category" name="category" required>
                ...
                <option value="exam" {{ old('category') == 'exam' ? 'selected' : '' }}>{{ __('Exam') }}</option>
                ...
                <option value="hostel" {{ old('category') == 'hostel' ? 'selected' : '' }}>{{ __('Hostel') }}</option>
                <option value="uniform" {{ old('category') == 'uniform' ? 'selected' : '' }}>{{ __('Uniform') }}</option>
                <option value="activity" {{ old('category') == 'activity' ? 'selected' : '' }}>{{ __('Activity') }}</option>
          ...
          <div class="row">
            ...
            <div class="col-md-6 mb-3">
              ...
              <select class="form-select @error('recurring_type') is-invalid @enderror" id="recurring_type" name="recurring_type" required>
                <option value="one-time" {{ old('recurring_type') == 'one-time' ? 'selected' : '' }}>{{ __('One-time') }}</option>
                <option value="monthly" {{ old('recurring_type') == 'monthly' ? 'selected' : '' }}>{{ __('Monthly') }}</option>
                <option value="quarterly" {{ old('recurring_type') == 'quarterly' ? 'selected' : '' }}>{{ __('Quarterly') }}</option>
          ...
          <div class="mb-3">
            ...
            <select class="form-select @error('applicable_to') is-invalid @enderror" 
              ...
              <option value="all" {{ old('applicable_to') == 'all' ? 'selected' : '' }}>{{ __('All Students') }}</option>
          ...
          <div class="d-flex gap-2">
            <button type="submit" class="btn btn-primary">
              <i class="bi bi-check-circle me-1"></i>{{ __('Create Fee') }}
            </button>
            <a href="{{ route('tenant.modules.fees.index') }}" class="btn btn-outline-secondary">
              {{ __('Cancel') }}
            </a>
