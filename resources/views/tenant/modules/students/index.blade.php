    ...
    <div class="card shadow-sm">
      <div class="card-body">
        ...
        <form method="get" class="row g-2 mb-3">
          <div class="col-auto">
            <input type="text" name="q" value="{{ $q ?? '' }}" class="form-control" placeholder="{{ __('Search name, admission, email') }}" />
          </div>
          <div class="col-auto">
            ...
            <tbody>
              @forelse($students as $s)
                <tr>
                  ...
                  <td>{{ $s->dob }}</td>
                  <td class="text-end">
                    <a class="btn btn-sm btn-outline-secondary" href="{{ route('tenant.modules.students.show', $s) }}">{{ __('Show') }}</a>
                    @can('update', $s)
                      <a class="btn btn-sm btn-outline-primary" href="{{ route('tenant.modules.students.edit', $s) }}">{{ __('Edit') }}</a>
                    @endcan
                    @can('delete', $s)
                      <form action="{{ route('tenant.modules.students.destroy', $s) }}" method="post" class="d-inline" onsubmit="return confirm('{{ __('Delete this student?') }}');">
                        @csrf
                        @method('DELETE')
              ...
              @empty
                <tr><td colspan="6" class="text-center text-muted py-4">{{ __('No students yet.') }}</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
