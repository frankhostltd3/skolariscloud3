                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <input type="text" name="search" class="form-control"
                                placeholder="Search by user or book..." value="{{ request('search') }}">
                        </div>
                        <div class="col-md-4">
                                ...
                                <tr>
                                        ...
                                        @if ($transaction->status === 'borrowed')
                                            @if ($transaction->isOverdue())
                                                <span class="badge bg-danger">Overdue
                                                    ({{ $transaction->days_overdue }}d)</span>
                                            @else
                                    ...
                                    <td>
                                        @if ($transaction->fine_amount > 0)
                                            <span
                                                class="text-danger fw-semibold">{{ format_money($transaction->fine_amount) }}</span>
                                            ...
                                            <div class="modal fade" id="returnModal{{ $transaction->id }}" tabindex="-1">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <form
                                                            action="{{ route('tenant.modules.library.transactions.return', $transaction) }}"
                                                            method="POST">
                                                            @csrf
                                                            ...
                                                            <div class="modal-body">
                                                                <p><strong>Book:</strong> {{ $transaction->book->title }}
                                                                </p>
                                                                ...
                                                                @if ($transaction->isOverdue())
                                                                    <div class="alert alert-warning">
                                                                        ...
                                                                        Suggested fine:
                                                                        {{ format_money($transaction->calculateFine()) }}
                                                                    </div>
                                                                @endif
                                                                <div class="mb-3">
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center py-5">
                                        ...
                                        @can('library.issue')
                                            <a href="{{ route('tenant.modules.library.transactions.borrow') }}"
