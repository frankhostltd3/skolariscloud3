            ...
            <div class="col-lg-8">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item"><a
                                    href="{{ route('tenant.modules.library.books.index') }}">Books</a></li>
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body p-4">
                        <form
                            action="{{ isset($book) ? route('tenant.modules.library.books.update', $book) : route('tenant.modules.library.books.store') }}"
                            ...
                            <div class="row g-3">
                                <!-- Title -->
                                <div class="col-md-12">
                                    <label class="form-label fw-semibold">Book Title <span
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="number" name="purchase_price"
                                        ...
                                        value="{{ old('purchase_price', $book->purchase_price ?? '') }}" step="0.01"
                                        min="0">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="status" class="form-select @error('status') is-invalid @enderror"
                                            ...
                                            <option value="maintenance"
                                                ...
                                                Maintenance</option>
                                            <option value="lost"
                                                {{ old('status', $book->status) === 'lost' ? 'selected' : '' }}>Lost
                                            </option>
                                            <option value="damaged"
                                                {{ old('status', $book->status) === 'damaged' ? 'selected' : '' }}>Damaged
                                            </option>
                                        </select>
                                        @error('status')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="col-md-12">
                                    <div class="form-check">
                                        <input type="checkbox" name="is_for_sale" class="form-check-input"
                                            id="is_for_sale" value="1"
                                            {{ old('is_for_sale', $book->is_for_sale ?? false) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="is_for_sale">
                                            <strong>Available for Sale in Bookstore</strong>
                                            <small class="d-block text-muted">Check this to make the book available for
                                                purchase</small>
                                        </label>
                                    </div>
                                </div>

                                <div id="bookstore-fields"
                                    style="display: {{ old('is_for_sale', $book->is_for_sale ?? false) ? 'contents' : 'none' }}">
                                    <!-- Cover Image Upload -->
                                    <div class="col-md-12">
                                    ...
                                    <!-- Sale Price -->
                                    <div class="col-md-4">
                                        <label class="form-label fw-semibold">Sale Price <span
                                                class="text-danger">*</span></label>
                                        <div class="input-group">
                                            <span class="input-group-text">$</span>
                                            <input type="number" name="sale_price"
                                                class="form-control @error('sale_price') is-invalid @enderror"
                                                value="{{ old('sale_price', $book->sale_price ?? '') }}" step="0.01"
                                                min="0">
                                        </div>
                                        @error('sale_price')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                    ...
                                    <!-- Stock Quantity -->
                                    <div class="col-md-4">
                                        <label class="form-label fw-semibold">Stock Quantity <span
                                                class="text-danger">*</span></label>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-check">
                                            ...
                                            <label class="form-check-label" for="is_featured">
                                                ...
                                                <small class="d-block text-muted">Display this book prominently in the
                                                    bookstore</small>
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-check">
                                            <input type="checkbox" name="is_digital" class="form-check-input"
                                                id="is_digital" value="1"
                                                {{ old('is_digital', $book->is_digital ?? false) ? 'checked' : '' }}>
                                            <label class="form-check-label" for="is_digital">
                                                <strong>Digital Product (E-Book)</strong>
                                                <small class="d-block text-muted">Check this if the book is a digital
                                                    download</small>
                                            </label>
                                        </div>
                                    </div>

                                    <div id="digital-fields" class="col-md-12"
                                        style="display: {{ old('is_digital', $book->is_digital ?? false) ? 'block' : 'none' }}">
                                        <label class="form-label fw-semibold">Digital File (PDF, EPUB, MOBI)</label>
                                        <input type="file" name="digital_file"
                                            class="form-control @error('digital_file') is-invalid @enderror"
                                            accept=".pdf,.epub,.mobi">
                                        @if (isset($book) && $book->digital_file_path)
                                            <div class="mt-2">
                                                <span class="badge bg-success"><i class="bi bi-check-circle me-1"></i>File
                                                    Uploaded</span>
    ...
    @push('scripts')
        <script>
            ...
            document.getElementById('is_digital').addEventListener('change', function() {
                ...
                digitalFields.style.display = this.checked ? 'block' : 'none';
            });
