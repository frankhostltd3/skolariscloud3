    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form id="idGenerationForm" method="POST" action="{{ route('tenant.modules.human_resources.employee-ids.generate') }}">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('employee_id') is-invalid @enderror" id="employee_id" name="employee_id" required>
                                        ...
                                        <option value="{{ $employee->id }}" {{ old('employee_id') == $employee->id ? 'selected' : '' }}>
                                            {{ $employee->first_name }} {{ $employee->last_name }} - {{ $employee->department?->name ?? __('No Department') }}
                                        </option>
                                        @endforeach
                                    </select>
                                    @error('employee_id')
...
function previewId() {
    ...
    fetch('{{ route("tenant.modules.human_resources.employee-ids.preview") }}', {
        method: 'POST',
        body: formData,
        headers: {
...
function downloadSvg() {
    ...
    const url = '{{ route("tenant.modules.human_resources.employee-ids.download.svg") }}' +
...
function downloadPng() {
    ...
    const url = '{{ route("tenant.modules.human_resources.employee-ids.download.png") }}' +
                '?employee_id=' + formData.get('employee_id') +
                '&template_id=' + formData.get('template_id');
