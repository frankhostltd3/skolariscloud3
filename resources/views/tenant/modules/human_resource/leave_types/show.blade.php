        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            ...
            <div>
                ...
                <a href="{{ route('tenant.modules.human-resource.leave-types.index') }}"
                    class="btn btn-secondary btn-sm">{{ __('Back to List') }}</a>
            </div>
        </div>
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <dl class="row">
                    ...
                    <dd class="col-sm-9">{{ strip_tags($leaveType->description) ?: '-' }}</dd>

                    <dt class="col-sm-3">{{ __('Created At') }}</dt>
                    <dd class="col-sm-9">{{ $leaveType->created_at ? $leaveType->created_at->format('M d, Y H:i') : '-' }}
                    </dd>

                    <dt class="col-sm-3">{{ __('Updated At') }}</dt>
                    <dd class="col-sm-9">{{ $leaveType->updated_at ? $leaveType->updated_at->format('M d, Y H:i') : '-' }}
                    </dd>
                </dl>
            </div>
        </div>
