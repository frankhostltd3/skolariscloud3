...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('tenant.modules.human-resource.leave-types.create') }}"
            class="btn btn-primary btn-sm">{{ __('Add leave type') }}</a>
    </div>
    ...
    <div class="mb-3">
        <a href="{{ route('tenant.modules.human-resource.leave-types.exportTemplate') }}"
            class="btn btn-info btn-sm">{{ __('Download Excel Template') }}</a>
        <a href="{{ route('tenant.modules.human-resource.leave-types.exportSqlTemplate') }}"
            class="btn btn-outline-info btn-sm">{{ __('Download SQL Template') }}</a>
        <button class="btn btn-success btn-sm" data-bs-toggle="modal"
            data-bs-target="#importModal">{{ __('Import Excel/CSV/SQL') }}</button>
        <a href="{{ route('tenant.modules.human-resource.leave-types.export', ['format' => 'excel']) }}"
            class="btn btn-warning btn-sm">{{ __('Export Excel') }}</a>
        <a href="{{ route('tenant.modules.human-resource.leave-types.export', ['format' => 'pdf']) }}"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="importFormat" id="formatExcel"
                                    value="excel" checked>
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="importFormat" id="formatSql"
                                    value="sql">
                    ...
                    <div id="excelImportForm">
                        <form method="POST"
                            action="{{ route('tenant.modules.human-resource.leave-types.import', 'excel') }}"
                    ...
                    <div id="sqlImportForm" style="display: none;">
                        <form method="POST"
                            action="{{ route('tenant.modules.human-resource.leave-types.import', 'sql') }}"
                            enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
                                <label for="sqlFile" class="form-label">{{ __('Select SQL file') }}</label>
                                <input type="file" class="form-control" id="sqlFile" name="file" accept=".sql"
                                    required>
                                <div class="form-text">
                                    {{ __('Upload a SQL file containing INSERT statements for the leave_types table') }}<br>
                                    <small
                                        class="text-muted">{{ __('Example: INSERT INTO leave_types (name, code, default_days, requires_approval, description) VALUES (\'Annual Leave\', \'AL\', 25, 1, \'Annual leave allowance\');') }}</small>
                                </div>
                            </div>
                            <div class="alert alert-warning">
                                <strong>{{ __('Security Note:') }}</strong>
                                {{ __('Only INSERT statements for the leave_types table will be processed. All other SQL statements will be ignored.') }}
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary"
                                    data-bs-dismiss="modal">{{ __('Close') }}</button>
            ...
            <form method="GET" class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control" id="search" name="search"
                        value="{{ request('search') }}" placeholder="{{ __('Search by name, code, or description') }}">
                </div>
                <div class="col-md-6 d-flex align-items-end">
                    <button type="submit" class="btn btn-outline-primary me-2">{{ __('Search') }}</button>
                    <a href="{{ route('tenant.modules.human-resource.leave-types.index') }}"
                ...
                <tbody>
                    @forelse($leaveTypes as $leaveType)
                        <tr>
                            ...
                            <td>{{ Str::limit(strip_tags($leaveType->description), 50) }}</td>
                            <td>
                                <div class="btn-group btn-group-sm" role="group">
                                    <a href="{{ route('tenant.modules.human-resource.leave-types.show', $leaveType) }}"
                                        class="btn btn-outline-info" title="{{ __('View') }}">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    <a href="{{ route('tenant.modules.human-resource.leave-types.edit', $leaveType) }}"
                                        class="btn btn-outline-warning" title="{{ __('Edit') }}">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <form method="POST"
                                        action="{{ route('tenant.modules.human-resource.leave-types.destroy', $leaveType) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center text-secondary">
                                {{ __('No leave types configured yet') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
