...
@section('content')
    <div class="container mt-4">
        ...
        <form method="POST" action="{{ route('tenant.modules.human-resource.positions.update', $position) }}">
            @csrf
            @method('PUT')
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control wysiwyg-editor @error('description') is-invalid @enderror" id="description"
                    name="description" data-placeholder="{{ __('Enter position description...') }}">{{ old('description', $position->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
