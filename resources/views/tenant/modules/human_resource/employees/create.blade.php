...
<div class="container-fluid">
  <div class="d-flex align-items-center justify-content-between mb-4">
    <div>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-2">
          ...
          <li class="breadcrumb-item"><a href="{{ route('tenant.modules.human_resources.employees.index') }}">{{ __('Employees') }}</a></li>
    ...
    <a href="{{ route('tenant.modules.human_resources.employees.index') }}" class="btn btn-outline-secondary">
  ...
  <form method="POST" action="{{ route('tenant.modules.human_resources.employees.store') }}" enctype="multipart/form-data" id="employeeForm">
    @csrf
    <div class="row">
      <!-- Main Content -->
      <div class="col-lg-8">
            ...
            <div class="row g-3">
              ...
              <div class="col-md-6">
                ...
                <input type="date" class="form-control @error('birth_date') is-invalid @enderror" 
                  id="birth_date" name="birth_date" value="{{ old('birth_date') }}" max="{{ date('Y-m-d', strtotime('-18 years')) }}">
        ...
        <div class="card shadow-sm mb-4">
          <div class="card-header bg-info text-white">
            <h5 class="card-title mb-0"><i class="bi bi-envelope me-2"></i>{{ __('Contact Information') }}</h5>
          </div>
          <div class="card-body">
            <div class="row g-3">
              <div class="col-md-6">
        ...
        <div class="card shadow-sm mb-4">
          <div class="card-header bg-success text-white">
            <h5 class="card-title mb-0"><i class="bi bi-briefcase me-2"></i>{{ __('Professional Information') }}</h5>
          </div>
          <div class="card-body">
            <div class="alert alert-info small">
              <i class="bi bi-info-circle me-2"></i>{{ __('Employee number will be auto-generated based on department') }}
            </div>
            <div class="row g-3">
              <div class="col-md-6">
              ...
              <div class="col-md-6">
                <label for="employee_type" class="form-label">{{ __('Employment Type') }} <span class="text-danger">*</span></label>
                <select class="form-select @error('employee_type') is-invalid @enderror" id="employee_type" name="employee_type" required>
                  <option value="">{{ __('Select Type') }}</option>
                  <option value="full_time" {{ old('employee_type')==='full_time' ? 'selected' : '' }}>{{ __('Full Time') }}</option>
                  <option value="part_time" {{ old('employee_type')==='part_time' ? 'selected' : '' }}>{{ __('Part Time') }}</option>
                  <option value="intern" {{ old('employee_type')==='intern' ? 'selected' : '' }}>{{ __('Intern') }}</option>
                  <option value="contract" {{ old('employee_type')==='contract' ? 'selected' : '' }}>{{ __('Contract') }}</option>
                </select>
              ...
              <div class="col-md-6">
                ...
                <select class="form-select @error('salary_scale_id') is-invalid @enderror" id="salary_scale_id" name="salary_scale_id">
                  <option value="">{{ __('Select Salary Scale') }}</option>
                  @foreach($salary_scales as $scale)
                    <option value="{{ $scale->id }}" {{ old('salary_scale_id') == $scale->id ? 'selected' : '' }}>
                      {{ $scale->name }}
                    </option>
                  @endforeach
                </select>
                @error('salary_scale_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
              </div>
              <div class="col-md-6">
        ...
        <div class="card shadow-sm mb-4">
          ...
          <div class="card-body">
            <div class="row g-3">
              <div class="col-md-4">
                <label for="emergency_contact_name" class="form-label">{{ __('Contact Name') }}</label>
                <input type="text" class="form-control @error('emergency_contact_name') is-invalid @enderror" 
                  id="emergency_contact_name" name="emergency_contact_name" value="{{ old('emergency_contact_name') }}">
                @error('emergency_contact_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
              </div>
              <div class="col-md-4">
                <label for="emergency_contact_phone" class="form-label">{{ __('Contact Phone') }}</label>
              ...
              <div class="col-md-4">
                <label for="emergency_contact_relation" class="form-label">{{ __('Relationship') }}</label>
                <input type="text" class="form-control @error('emergency_contact_relation') is-invalid @enderror" 
                  id="emergency_contact_relation" name="emergency_contact_relation" value="{{ old('emergency_contact_relation') }}">
        ...
        <div class="card shadow-sm mb-4">
          <div class="card-header bg-light">
            <h5 class="card-title mb-0"><i class="bi bi-file-text me-2"></i>{{ __('Additional Information') }}</h5>
          </div>
          <div class="card-body">
            <div class="row g-3">
              <div class="col-12">
                ...
                <textarea class="form-control @error('medical_conditions') is-invalid @enderror" 
                  id="medical_conditions" name="medical_conditions" rows="2">{{ old('medical_conditions') }}</textarea>
      ...
      <div class="col-lg-4">
        ...
        <div class="card shadow-sm mb-4 sticky-top" style="top: 20px;">
          <div class="card-header bg-light">
            <h5 class="card-title mb-0"><i class="bi bi-gear me-2"></i>{{ __('Employment Status') }}</h5>
          </div>
          <div class="card-body">
            <div class="mb-3">
              ...
              <select class="form-select @error('employment_status') is-invalid @enderror" id="employment_status" name="employment_status">
                <option value="active" {{ old('employment_status', 'active')==='active' ? 'selected' : '' }}>{{ __('Active') }}</option>
                <option value="probation" {{ old('employment_status')==='probation' ? 'selected' : '' }}>{{ __('Probation') }}</option>
                <option value="on_leave" {{ old('employment_status')==='on_leave' ? 'selected' : '' }}>{{ __('On Leave') }}</option>
                <option value="inactive" {{ old('employment_status')==='inactive' ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                <option value="suspended" {{ old('employment_status')==='suspended' ? 'selected' : '' }}>{{ __('Suspended') }}</option>
        ...
        <div class="card shadow-sm mb-4">
          ...
          <div class="card-body">
            <ul class="list-unstyled mb-0 small">
              ...
              <li class="mb-2"><i class="bi bi-check-circle text-success me-2"></i>{{ __('Employee number is auto-generated') }}</li>
              <li class="mb-2"><i class="bi bi-check-circle text-success me-2"></i>{{ __('Photo max size: 2MB') }}</li>
              <li class="mb-2"><i class="bi bi-check-circle text-success me-2"></i>{{ __('Department determines employee number prefix') }}</li>
