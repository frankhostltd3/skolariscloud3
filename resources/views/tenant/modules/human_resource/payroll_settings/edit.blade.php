    ...
    <div class="container">
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                ...
                <div class="small text-secondary">{{ __('Configure payroll policies, rates, and automation settings') }}
                </div>
            </div>
        ...
        <form method="POST" action="{{ route('tenant.modules.human-resource.payroll-settings.update') }}">
            @csrf
            @method('PUT')
            ...
            <ul class="nav nav-tabs mb-4" id="payrollTabs" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="pay-period-tab" data-bs-toggle="tab" data-bs-target="#pay-period"
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="pay_frequency" name="pay_frequency">
                                            @foreach ($payFreqOptions as $key => $label)
                                                <option value="{{ $key }}"
                                                    {{ ($currentSettings['pay_frequency'] ?? '') == $key ? 'selected' : '' }}>
                                                    {{ $label }}</option>
                                            @endforeach
                                        </select>
                                        <div class="form-text">
                                            {{ $groupedSettings['pay_period']['pay_frequency']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control" id="pay_day" name="pay_day"
                                            value="{{ $currentSettings['pay_day'] ?? '' }}" min="1"
                                            max="31">
                                        <div class="form-text">
                                            {{ $groupedSettings['pay_period']['pay_day']['description'] ?? '' }}</div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="fiscal_year_start"
                                            class="form-label">{{ __('Fiscal Year Start') }}</label>
                                        <input type="text" class="form-control" id="fiscal_year_start"
                                            name="fiscal_year_start"
                                            value="{{ $currentSettings['fiscal_year_start'] ?? '' }}"
                                            placeholder="MM-DD">
                                        <div class="form-text">
                                            {{ $groupedSettings['pay_period']['fiscal_year_start']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="default_currency"
                                            class="form-label">{{ __('Default Currency') }}</label>
                                        @php
                                            $currencyOptions =
                                                $groupedSettings['currency']['default_currency']['options'] ?? [];
                                        ...
                                        <select class="form-select" id="default_currency" name="default_currency">
                                            @foreach ($currencyOptions as $code => $label)
                                                <option value="{{ $code }}"
                                                    {{ ($currentSettings['default_currency'] ?? '') == $code ? 'selected' : '' }}>
                                                    {{ $label }}</option>
                                            @endforeach
                                        </select>
                                        <div class="form-text">
                                            {{ $groupedSettings['currency']['default_currency']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <div class="form-text">
                                            {{ $groupedSettings['currency']['currency_symbol']['description'] ?? 'Symbol automatically derived from default currency (e.g., $ for USD, £ for GBP, USh for UGX). Override if needed.' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control" id="decimal_places"
                                            name="decimal_places" value="{{ $currentSettings['decimal_places'] ?? '' }}"
                                            min="0" max="4">
                                        <div class="form-text">
                                            {{ $groupedSettings['currency']['decimal_places']['description'] ?? '' }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card shadow-sm">
                        ...
                        <div class="card-body">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="basic_salary_percentage"
                                            class="form-label">{{ __('Basic Salary (%)') }}</label>
                                        <input type="number" class="form-control" id="basic_salary_percentage"
                                            name="basic_salary_percentage"
                                            value="{{ $currentSettings['basic_salary_percentage'] ?? '' }}"
                                            min="0" max="100" step="0.01">
                                        <div class="form-text">
                                            {{ $groupedSettings['salary_components']['basic_salary_percentage']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="house_allowance_percentage"
                                            class="form-label">{{ __('House Allowance (%)') }}</label>
                                        <input type="number" class="form-control" id="house_allowance_percentage"
                                            name="house_allowance_percentage"
                                            value="{{ $currentSettings['house_allowance_percentage'] ?? '' }}"
                                            min="0" max="100" step="0.01">
                                        <div class="form-text">
                                            {{ $groupedSettings['salary_components']['house_allowance_percentage']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="transport_allowance_percentage"
                                            class="form-label">{{ __('Transport Allowance (%)') }}</label>
                                        <input type="number" class="form-control" id="transport_allowance_percentage"
                                            name="transport_allowance_percentage"
                                            value="{{ $currentSettings['transport_allowance_percentage'] ?? '' }}"
                                            min="0" max="100" step="0.01">
                                        <div class="form-text">
                                            {{ $groupedSettings['salary_components']['transport_allowance_percentage']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="medical_allowance_percentage"
                                            class="form-label">{{ __('Medical Allowance (%)') }}</label>
                                        <input type="number" class="form-control" id="medical_allowance_percentage"
                                            name="medical_allowance_percentage"
                                            value="{{ $currentSettings['medical_allowance_percentage'] ?? '' }}"
                                            min="0" max="100" step="0.01">
                                        <div class="form-text">
                                            {{ $groupedSettings['salary_components']['medical_allowance_percentage']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="mt-3">
                                <strong>{{ __('Total:') }}</strong> <span id="total-percentage"
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="income_tax_rate"
                                            class="form-label">{{ __('Income Tax Rate (%)') }}</label>
                                        <input type="number" class="form-control" id="income_tax_rate"
                                            name="income_tax_rate"
                                            value="{{ $currentSettings['income_tax_rate'] ?? '' }}" min="0"
                                            max="100" step="0.01">
                                        <div class="form-text">
                                            {{ $groupedSettings['deductions']['income_tax_rate']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="social_security_rate"
                                            class="form-label">{{ __('Social Security Rate (%)') }}</label>
                                        <input type="number" class="form-control" id="social_security_rate"
                                            name="social_security_rate"
                                            value="{{ $currentSettings['social_security_rate'] ?? '' }}" min="0"
                                            max="50" step="0.01">
                                        <div class="form-text">
                                            {{ $groupedSettings['deductions']['social_security_rate']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="provident_fund_rate"
                                            class="form-label">{{ __('Provident Fund Rate (%)') }}</label>
                                        <input type="number" class="form-control" id="provident_fund_rate"
                                            name="provident_fund_rate"
                                            value="{{ $currentSettings['provident_fund_rate'] ?? '' }}" min="0"
                                            max="50" step="0.01">
                                        <div class="form-text">
                                            {{ $groupedSettings['deductions']['provident_fund_rate']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card shadow-sm">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="overtime_rate_regular"
                                            class="form-label">{{ __('Regular Overtime Rate') }}</label>
                                        <input type="number" class="form-control" id="overtime_rate_regular"
                                            name="overtime_rate_regular"
                                            value="{{ $currentSettings['overtime_rate_regular'] ?? '' }}" min="1"
                                            max="5" step="0.1">
                                        <div class="form-text">
                                            {{ $groupedSettings['overtime']['overtime_rate_regular']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="overtime_rate_holiday"
                                            class="form-label">{{ __('Holiday Overtime Rate') }}</label>
                                        <input type="number" class="form-control" id="overtime_rate_holiday"
                                            name="overtime_rate_holiday"
                                            value="{{ $currentSettings['overtime_rate_holiday'] ?? '' }}" min="1"
                                            max="5" step="0.1">
                                        <div class="form-text">
                                            {{ $groupedSettings['overtime']['overtime_rate_holiday']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="max_overtime_hours_monthly"
                                            class="form-label">{{ __('Max Overtime Hours (Monthly)') }}</label>
                                        <input type="number" class="form-control" id="max_overtime_hours_monthly"
                                            name="max_overtime_hours_monthly"
                                            value="{{ $currentSettings['max_overtime_hours_monthly'] ?? '' }}"
                                            min="0" max="200">
                                        <div class="form-text">
                                            {{ $groupedSettings['overtime']['max_overtime_hours_monthly']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <div class="form-text">
                                            {{ $groupedSettings['banking']['bank_name']['description'] ?? '' }}</div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <div class="form-text">
                                            {{ $groupedSettings['banking']['bank_account_number']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="payment_method"
                                            class="form-label">{{ __('Default Payment Method') }}</label>
                                        @php
                                        ...
                                        <select class="form-select" id="payment_method" name="payment_method">
                                            @foreach ($paymentMethodOptions as $key => $label)
                                                <option value="{{ $key }}"
                                                    {{ ($currentSettings['payment_method'] ?? 'bank_transfer') == $key ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <div class="form-text">
                                            {{ $groupedSettings['banking']['payment_method']['description'] ?? 'Primary method for salary payments to employees.' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="minimum_wage"
                                            class="form-label">{{ __('Minimum Monthly Wage') }}</label>
                                        <div class="input-group">
                                            <span
                                                class="input-group-text">{{ $currentSettings['currency_symbol'] ?? '$' }}</span>
                                            <input type="number" class="form-control" id="minimum_wage"
                                                name="minimum_wage" value="{{ $currentSettings['minimum_wage'] ?? '' }}"
                                                min="0" step="0.01">
                                        </div>
                                        <div class="form-text">
                                            {{ $groupedSettings['compliance']['minimum_wage']['description'] ?? '' }}</div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="working_hours_per_day"
                                            class="form-label">{{ __('Working Hours Per Day') }}</label>
                                        <input type="number" class="form-control" id="working_hours_per_day"
                                            name="working_hours_per_day"
                                            value="{{ $currentSettings['working_hours_per_day'] ?? '' }}" min="1"
                                            max="24">
                                        <div class="form-text">
                                            {{ $groupedSettings['compliance']['working_hours_per_day']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="working_days_per_week"
                                            class="form-label">{{ __('Working Days Per Week') }}</label>
                                        <input type="number" class="form-control" id="working_days_per_week"
                                            name="working_days_per_week"
                                            value="{{ $currentSettings['working_days_per_week'] ?? '' }}" min="1"
                                            max="7">
                                        <div class="form-text">
                                            {{ $groupedSettings['compliance']['working_days_per_week']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="mb-3 form-check">
                                        ...
                                        <input class="form-check-input" type="checkbox" id="auto_process_payroll"
                                            name="auto_process_payroll" value="1"
                                        ...
                                        <div class="form-text">
                                            {{ $groupedSettings['integration']['auto_process_payroll']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3 form-check">
                                        ...
                                        <input class="form-check-input" type="checkbox" id="email_pay_slips"
                                            name="email_pay_slips" value="1"
                                        ...
                                        <div class="form-text">
                                            {{ $groupedSettings['integration']['email_pay_slips']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3 form-check">
                                        ...
                                        <input class="form-check-input" type="checkbox" id="export_to_accounting"
                                            name="export_to_accounting" value="1"
                                        ...
                                        <div class="form-text">
                                            {{ $groupedSettings['integration']['export_to_accounting']['description'] ?? '' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="export_format" name="export_format">
                                            @foreach ($groupedSettings['integration']['export_format']['options'] ?? ['csv' => 'CSV', 'json' => 'JSON'] as $key => $label)
                                                <option value="{{ $key }}"
                                                    {{ ($currentSettings['export_format'] ?? 'csv') == $key ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <div class="form-text">
                                            {{ $groupedSettings['integration']['export_format']['description'] ?? 'File format for accounting exports' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="alert alert-info mt-3">
                                ...
                                <ul class="mb-0 mt-2">
                                    ...
                                    <li>Storage permissions for accounting exports (stored in
                                        <code>storage/app/exports/payroll/</code>)</li>
                                </ul>
                            </div>
                        </div>
                    </div>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4">
                <a href="{{ route('tenant.modules.human-resource.payroll-settings.index') }}" class="btn btn-secondary">
