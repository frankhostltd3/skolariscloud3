...
<div class="container">
  <div class="d-flex align-items-center justify-content-between mb-4">
    ...
    <a href="{{ route('tenant.modules.human_resources.leave_requests.index') }}" class="btn btn-secondary btn-sm">{{ __('Back to Leave Requests') }}</a>
  </div>
  ...
  <div class="row">
    <div class="col-lg-8">
      <div class="card shadow-sm">
        <div class="card-body">
          <form method="POST" action="{{ route('tenant.modules.human_resources.leave_requests.store') }}" id="leaveRequestForm">
            @csrf
                ...
                <select class="form-select" id="leave_type_id" name="leave_type_id" required>
                  ...
                  @foreach($leaveTypes as $leaveType)
                    <option value="{{ $leaveType->id }}" {{ old('leave_type_id') == $leaveType->id ? 'selected' : '' }}>
                      {{ $leaveType->name }} ({{ $leaveType->code }} - {{ $leaveType->default_days }} days)
                    </option>
                  @endforeach
                </select>
                @error('leave_type_id') <div class="text-danger">{{ $message }}</div> @enderror
              </div>
            </div>
            ...
            <div class="mb-3">
              <label for="reason" class="form-label">{{ __('Reason for Leave') }} <span class="text-danger">*</span></label>
            ...
            <div class="d-flex gap-2">
              ...
              <a href="{{ route('tenant.modules.human_resources.leave_requests.index') }}" class="btn btn-secondary">
      ...
      <div class="card shadow-sm">
        ...
        <div class="card-body">
          <ul class="list-unstyled mb-0 small">
            <li class="mb-2"><i class="bi bi-check-circle text-success"></i> {{ __('Submit requests at least 2 weeks in advance') }}</li>
            <li class="mb-2"><i class="bi bi-check-circle text-success"></i> {{ __('Provide clear reason for your leave') }}</li>
            <li class="mb-2"><i class="bi bi-check-circle text-success"></i> {{ __('Check your leave balance before requesting') }}</li>
            <li class="mb-2"><i class="bi bi-check-circle text-success"></i> {{ __('Emergency leave may require immediate approval') }}</li>
          </ul>
        </div>
      </div>
