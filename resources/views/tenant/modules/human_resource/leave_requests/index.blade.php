...
<div class="d-flex align-items-center justify-content-between mb-4">
  <div>
    ...
    <div class="small text-secondary">{{ __('Review, approve, or decline staff leave requests with financial tracking.') }}</div>
  </div>
  <div class="d-flex gap-2">
    <a href="{{ route('tenant.modules.human_resources.leave_requests.create') }}" class="btn btn-primary btn-sm">
      <i class="bi bi-plus-circle"></i> {{ __('New Leave Request') }}
    </a>
    <a href="{{ route('tenant.modules.human_resources.leave_requests.financial_report') }}" class="btn btn-outline-info btn-sm">
      <i class="bi bi-graph-up"></i> {{ __('Financial Report') }}
    </a>
    <a href="{{ route('tenant.modules.human_resources.leave_requests.export_financial', request()->all()) }}" class="btn btn-outline-success btn-sm">
  ...
  <div class="card-body">
    <form method="GET" class="row g-3">
      ...
      <div class="col-md-3">
        ...
        <input type="text" class="form-control form-control-sm" id="employee_filter" name="employee" value="{{ request('employee') }}" placeholder="{{ __('Search by name') }}">
      </div>
      <div class="col-md-2">
        <label for="leave_type_filter" class="form-label">{{ __('Leave Type') }}</label>
        <input type="text" class="form-control form-control-sm" id="leave_type_filter" name="leave_type" value="{{ request('leave_type') }}" placeholder="{{ __('Type or code') }}">
      </div>
      <div class="col-md-2">
      ...
      <div class="col-md-1 d-flex align-items-end">
        ...
        <a href="{{ route('tenant.modules.human_resources.leave_requests.index') }}" class="btn btn-outline-secondary btn-sm">{{ __('Clear') }}</a>
      </div>
    </form>
  </div>
</div>
          ...
          <tr>
            <td>
              <div class="d-flex align-items-center">
                ...
                <div>
                  <div class="fw-semibold">{{ $leaveRequest->employee->first_name }} {{ $leaveRequest->employee->last_name }}</div>
            ...
            <td>
              @if($leaveRequest->is_paid)
                ...
                <br><small class="text-muted">{{ format_money($leaveRequest->financial_impact ?? 0) }}</small>
              @else
                <span class="text-danger">
                  <i class="bi bi-dash-circle"></i> {{ __('Unpaid') }}
                </span>
                <br><small class="text-danger">-{{ format_money($leaveRequest->financial_impact ?? 0) }}</small>
              @endif
            </td>
            <td>
            ...
            <td>
              <div class="btn-group btn-group-sm" role="group">
                <a href="{{ route('tenant.modules.human_resources.leave_requests.show', $leaveRequest) }}" class="btn btn-outline-info" title="{{ __('View Details') }}">
                  <i class="bi bi-eye"></i>
                </a>
                @if($leaveRequest->status === 'pending')
                  <form method="POST" action="{{ route('tenant.modules.human_resources.leave_requests.approve', $leaveRequest) }}" style="display: inline;">
                    @csrf
                    <button type="submit" class="btn btn-outline-success" title="{{ __('Approve') }}" onclick="return confirm('{{ __('Approve this leave request?') }}')">
                      <i class="bi bi-check-lg"></i>
                    </button>
                  </form>
                  <form method="POST" action="{{ route('tenant.modules.human_resources.leave_requests.reject', $leaveRequest) }}" style="display: inline;">
                    @csrf
                    <button type="submit" class="btn btn-outline-danger" title="{{ __('Decline') }}" onclick="return confirm('{{ __('Decline this leave request?') }}')">
        ...
        @empty
          <tr>
            <td colspan="7" class="text-center text-secondary py-4">
              ...
              <a href="{{ route('tenant.modules.human_resources.leave_requests.index') }}" class="btn btn-outline-secondary btn-sm mt-2">
                {{ __('Clear Filters') }}
              </a>
            </td>
          </tr>
