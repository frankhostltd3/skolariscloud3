...
<div class="container">
  <div class="d-flex align-items-center justify-content-between mb-4">
    ...
    <a href="{{ route('tenant.modules.human_resources.leave_requests.show', $leaveRequest) }}" class="btn btn-secondary btn-sm">
    ...
    <div class="col-lg-8">
      ...
      <div class="card shadow-sm mb-4">
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-md-6">
              <p class="mb-1"><strong>{{ __('Employee:') }}</strong> {{ $leaveRequest->employee->first_name }} {{ $leaveRequest->employee->last_name }}</p>
              <p class="mb-1"><strong>{{ __('Leave Type:') }}</strong> {{ $leaveRequest->leaveType->name }}</p>
              <p class="mb-0"><strong>{{ __('Days Requested:') }}</strong> {{ $leaveRequest->days_requested }}</p>
            </div>
            <div class="col-md-6">
              <p class="mb-1"><strong>{{ __('From:') }}</strong> {{ $leaveRequest->start_date->format('M d, Y') }}</p>
              <p class="mb-1"><strong>{{ __('To:') }}</strong> {{ $leaveRequest->end_date->format('M d, Y') }}</p>
      ...
      <div class="card shadow-sm">
        ...
        <div class="card-body">
          <form method="POST" action="{{ route('tenant.modules.human_resources.leave_requests.update', $leaveRequest) }}">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-6">
                  <div class="form-check form-check-lg">
                    ...
                    <label class="form-check-label" for="status_approved">
                      ...
                      <br><small class="text-muted">{{ __('Allow the employee to take the requested leave') }}</small>
                    </label>
                  </div>
                </div>
                <div class="col-md-6">
            ...
            <div class="mb-4">
              <label for="manager_comment" class="form-label">{{ __('Comments') }} <span class="text-danger">*</span></label>
              ...
              @error('manager_comment') <div class="text-danger">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="d-flex gap-2">
              ...
              <a href="{{ route('tenant.modules.human_resources.leave_requests.show', $leaveRequest) }}" class="btn btn-secondary">
      ...
      <div class="card shadow-sm mt-3">
        ...
        <div class="card-body">
          <ul class="list-unstyled mb-0 small">
            <li class="mb-2"><i class="bi bi-check-circle text-success"></i> {{ __('Check employee leave balance') }}</li>
            <li class="mb-2"><i class="bi bi-check-circle text-success"></i> {{ __('Verify leave type policies') }}</li>
            <li class="mb-2"><i class="bi bi-check-circle text-success"></i> {{ __('Consider business impact') }}</li>
            <li class="mb-2"><i class="bi bi-check-circle text-success"></i> {{ __('Provide clear feedback') }}</li>
          </ul>
        </div>
      </div>
      ...
      <div class="card shadow-sm mt-3">
        ...
        <div class="card-body">
          <div class="row text-center">
            <div class="col-6">
              <div class="h5 text-warning mb-1">{{ \App\Models\LeaveRequest::where('status', 'pending')->count() }}</div>
              <small class="text-muted">{{ __('Pending') }}</small>
            </div>
            <div class="col-6">
              <div class="h5 text-success mb-1">{{ \App\Models\LeaveRequest::where('status', 'approved')->count() }}</div>
