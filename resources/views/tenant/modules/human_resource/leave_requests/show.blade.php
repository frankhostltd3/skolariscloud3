        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            ...
            <div class="d-flex gap-2">
                @if ($leaveRequest->status === 'pending')
                    <a href="{{ route('tenant.modules.human-resource.leave-requests.edit', $leaveRequest) }}"
                        class="btn btn-warning btn-sm">
                        <i class="bi bi-pencil"></i> {{ __('Review Request') }}
                    </a>
                @endif
                <a href="{{ route('tenant.modules.human-resource.leave-requests.index') }}" class="btn btn-secondary btn-sm">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <div class="d-flex align-items-center mb-2">
                                    ...
                                    <div>
                                        <h6 class="mb-0">{{ $leaveRequest->employee->first_name }}
                                            {{ $leaveRequest->employee->last_name }}</h6>
                                        <small class="text-muted">{{ $leaveRequest->employee->employee_type }}</small>
                                    </div>
                                </div>
                                <p class="mb-1"><strong>{{ __('Email:') }}</strong>
                                    {{ $leaveRequest->employee->email }}</p>
                                <p class="mb-0"><strong>{{ __('Phone:') }}</strong>
                                    {{ $leaveRequest->employee->phone ?? 'Not provided' }}</p>
                            </div>

                            <div class="col-md-6">
                                <h6 class="text-muted mb-2">{{ __('Leave Details') }}</h6>
                                <p class="mb-1"><strong>{{ __('Leave Type:') }}</strong>
                                    {{ $leaveRequest->leaveType->name }} ({{ $leaveRequest->leaveType->code }})</p>
                                <p class="mb-1"><strong>{{ __('Start Date:') }}</strong>
                                    {{ $leaveRequest->start_date->format('l, F d, Y') }}</p>
                                <p class="mb-1"><strong>{{ __('End Date:') }}</strong>
                                    {{ $leaveRequest->end_date->format('l, F d, Y') }}</p>
                                <p class="mb-1"><strong>{{ __('Days Requested:') }}</strong>
                                    {{ $leaveRequest->days_requested }} working days</p>
                                <p class="mb-0"><strong>{{ __('Submitted:') }}</strong>
                            ...
                            <div class="d-grid gap-2">
                                <form method="POST"
                                    ...
                                    <button type="submit" class="btn btn-success w-100"
                                        onclick="return confirm('{{ __('Are you sure you want to approve this leave request?') }}')">
                                ...
                                <form method="POST"
                                    ...
                                    <button type="submit" class="btn btn-danger w-100"
                                        onclick="return confirm('{{ __('Are you sure you want to reject this leave request?') }}')">
