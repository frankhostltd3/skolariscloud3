...
<div class="card shadow-sm">
	...
	<div class="list-group list-group-flush">
		...
		<a
			class="list-group-item list-group-item-action d-flex justify-content-between align-items-center text-decoration-none {{ $academicsActive ? 'active' : '' }}"
			data-bs-toggle="collapse"
			href="#academicsMenuStaff"
			role="button"
			aria-expanded="{{ $academicsActive ? 'true' : 'false' }}"
			aria-controls="academicsMenuStaff"
		>
		...
		<div class="collapse {{ $academicsActive ? 'show' : '' }}" id="academicsMenuStaff">
			<div class="list-group list-group-flush ms-3">
				<a class="list-group-item list-group-item-action text-decoration-none {{ request()->routeIs('tenant.academics.classes.*') ? 'active' : '' }}" href="{{ route('tenant.academics.classes.index') }}">
					{{ __('Classes') }}
				</a>
				<a class="list-group-item list-group-item-action text-decoration-none {{ request()->routeIs('tenant.academics.class_streams.*') ? 'active' : '' }}" href="{{ route('tenant.academics.class_streams.index') }}">
					{{ __('Class streams') }}
				</a>
				<a class="list-group-item list-group-item-action text-decoration-none {{ request()->routeIs('tenant.academics.subjects.*') ? 'active' : '' }}" href="{{ route('tenant.academics.subjects.index') }}">
					{{ __('Subjects') }}
				</a>
				<a class="list-group-item list-group-item-action text-decoration-none {{ request()->routeIs('tenant.academics.timetable.*') ? 'active' : '' }}" href="{{ route('tenant.academics.timetable.index') }}">
					{{ __('Timetable') }}
				</a>
				<a class="list-group-item list-group-item-action text-decoration-none {{ request()->routeIs('tenant.academics.terms.*') ? 'active' : '' }}" href="{{ route('tenant.academics.terms.index') }}">
					{{ __('Terms') }}
				</a>
				@canany(['manage grades','view grades'])
					<a class="list-group-item list-group-item-action text-decoration-none {{ request()->routeIs('tenant.academics.grades.*') ? 'active' : '' }}" href="{{ route('tenant.academics.grades.index') }}">
						{{ __('Grades') }}
					</a>
				@endcanany
			</div>
		...
		<script>
			document.addEventListener('DOMContentLoaded', function() {
				...
				const scheduleHide = () => { hideTimer = setTimeout(() => menu.classList.remove('show'), 150); };
				const cancelHide = () => { if (hideTimer) { clearTimeout(hideTimer); hideTimer = null; } };
				trigger.addEventListener('mouseenter', () => { cancelHide(); showMenu(); });
...
<div class="card shadow-sm mb-4">
	<div class="card-body">
		...
		<div class="d-flex flex-wrap gap-2 small">
			<a class="btn btn-outline-secondary btn-sm" href="{{ route('tenant.storefront.home') }}">{{ __('Bookstore') }}</a>
		</div>
	</div>
</div>
