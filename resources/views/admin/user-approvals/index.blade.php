            ...
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            @if($status === 'pending')
                                <td><input type="checkbox" name="user_ids[]" value="{{ $user->id }}" class="user-checkbox" form="bulkActionForm"></td>
                            @endif
                            <td>
                            ...
                            @if($status === 'approved' || $status === 'rejected')
                                <td>
                                    ...
                                    <small class="text-muted">{{ $user->approved_at?->format('M d, Y H:i') }}</small>
                                </td>
                            @endif
                            <td class="text-end text-nowrap">
                                <a href="{{ route('admin.user-approvals.show', $user) }}" class="btn btn-sm btn-info me-1" title="{{ __('View Details') }}">
                                    <i class="bi bi-eye"></i> {{ __('View') }}
                                </a>
                                @if($status === 'pending')
                                    <form method="POST" action="{{ route('admin.user-approvals.approve', $user) }}" class="d-inline">
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-success me-1" title="{{ __('Approve') }}" onclick="return confirm('{{ __('Approve this user?') }}')">
                                            <i class="bi bi-check-circle"></i> {{ __('Approve') }}
                                        </button>
                                    </form>
                                    <button type="button" class="btn btn-sm btn-danger" title="{{ __('Reject') }}" data-bs-toggle="modal" data-bs-target="#rejectModal{{ $user->id }}">
                                            ...
                                            <div class="modal-content">
                                                <form method="POST" action="{{ route('admin.user-approvals.reject', $user) }}">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <textarea name="rejection_reason" class="form-control" rows="4" required placeholder="{{ __('Reason for rejection...') }}"></textarea>
                                                    </div>
                                ...
                                @elseif($status === 'approved')
                                    @php
                                        $isStaffOrTeacher = $user->hasRole('Staff') || $user->hasRole('Teacher');
                                    ...
                                    @if($showManage)
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                @if($isStaffOrTeacher)
                                                    <li>
                                                        <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#employmentModal{{ $user->id }}">
                                                            <i class="bi bi-briefcase me-1"></i> {{ __('Edit Employment') }}
                                                        </a>
                                                    </li>
                                                @endif
                                                ...
                                                @if($isStudent)
                                                    <li>
                                                        <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#studentEnrollmentModal{{ $user->id }}">
                                                            <i class="bi bi-book me-1"></i> {{ __('Edit Enrollment') }}
                                                        </a>
                                                    </li>
                                                @endif
                                                @if($isParent)
                                                    <li>
                                                        <a class="dropdown-item" href="{{ route('tenant.users.parents.edit', $user) }}">
                                                            <i class="bi bi-people-fill me-1"></i> {{ __('Link Children / Edit Guardian') }}
                                                        </a>
                                                    </li>
                                                @endif
                                            ...
                                            @if($user->is_active)
                                                <li>
                                                    <form method="POST" action="{{ route('admin.user-approvals.suspend', $user) }}" class="px-3 py-1">
                                                        @csrf
                                                        <div class="d-flex gap-2">
                                                            <input type="hidden" name="reason" value="{{ __('Suspended via approvals list') }}">
                                                            <button type="submit" class="btn btn-sm btn-warning w-100" onclick="return confirm('{{ __('Suspend this user?') }}')">
                                            ...
                                            @else
                                                <li>
                                                    <form method="POST" action="{{ route('admin.user-approvals.reinstate', $user) }}" class="px-3 py-1">
                                                        @csrf
                                                        <button type="submit" class="btn btn-sm btn-success w-100" onclick="return confirm('{{ __('Reinstate this user?') }}')">
                                            ...
                                            @endif
                                            <li>
                                                <form method="POST" action="{{ route('admin.user-approvals.expel', $user) }}" class="px-3 py-1">
                                                    @csrf
                                                    <div class="d-flex gap-2">
                                                        <input type="hidden" name="reason" value="{{ __('Expelled/Terminated via approvals list') }}">
                                                        <button type="submit" class="btn btn-sm btn-danger w-100" onclick="return confirm('{{ __('Expel/Terminate this user? This will clear teaching allocations.') }}')">
                                    ...
                                    @php
                                        ...
                                        $currentEmploymentType = $teacher->employment_type ?? ($employee->employee_type ?? '');
                                    @endphp
                                    <div class="modal fade" id="employmentModal{{ $user->id }}" tabindex="-1" aria-hidden="true">
                                        <div class="modal-dialog modal-lg">
                                            <div class="modal-content">
                                                <form method="POST" action="{{ route('admin.user-approvals.employment', $user) }}" id="employmentForm{{ $user->id }}">
                                                    @csrf
                                                    <div class="modal-header">
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="row g-3">
                                                            <div class="col-md-6">
                                                                ...
                                                                <select name="employment_role" class="form-select employment-role-select" required data-user-id="{{ $user->id }}">
                                                            ...
                                                            <div class="col-md-6">
                                                                ...
                                                                <select name="employee_type" class="form-select employment-type-dropdown" id="employmentTypeDropdown{{ $user->id }}" style="{{ $user->hasRole('Teacher') ? '' : 'display:none;' }}">
                                                                    <option value="full_time" {{ $currentEmploymentType === 'full_time' ? 'selected' : '' }}>{{ __('Full Time') }}</option>
                                                                    <option value="part_time" {{ $currentEmploymentType === 'part_time' ? 'selected' : '' }}>{{ __('Part Time') }}</option>
                                                                    <option value="contract" {{ $currentEmploymentType === 'contract' ? 'selected' : '' }}>{{ __('Contract') }}</option>
                                                                    <option value="visiting" {{ $currentEmploymentType === 'visiting' ? 'selected' : '' }}>{{ __('Visiting') }}</option>
                                                                </select>
                                                                <!-- Show text input for non-teachers -->
                                                                <input type="text" name="employee_type_text" class="form-control employment-type-text" id="employmentTypeText{{ $user->id }}" value="{{ $currentEmploymentType }}" placeholder="{{ __('e.g., full_time, administrative, etc.') }}" style="{{ $user->hasRole('Teacher') ? 'display:none;' : '' }}">
                                                            </div>
                                                            <div class="col-md-6">
                                                                <label class="form-label">{{ __('Department') }}</label>
                                                                <select name="department_id" class="form-select employment-department-select" data-user-id="{{ $user->id }}">
                                                            ...
                                                            <div class="col-md-6">
                                                                ...
                                                                <select name="position_id" class="form-select employment-position-select" data-user-id="{{ $user->id }}">
                                ...
                                @if($isStudent)
                                    @php
                                        ...
                                        $studentSubjects = $student ? $student->subjects->pluck('id')->toArray() : [];
                                    @endphp
                                    <div class="modal fade" id="studentEnrollmentModal{{ $user->id }}" tabindex="-1" aria-hidden="true">
                                        <div class="modal-dialog modal-lg">
                                            <div class="modal-content">
                                                <form method="POST" action="{{ route('admin.user-approvals.student-enrollment', $user) }}">
                                                    @csrf
                                                    <div class="modal-header">
                                                        <h5 class="modal-title">{{ __('Edit Enrollment for') }} {{ $user->name }}</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <div class="row g-3">
                                                            <div class="col-md-6">
                                                                ...
                                                                <select name="class_id" class="form-select student-class-select" id="studentClassSelect{{ $user->id }}" data-user-id="{{ $user->id }}" required>
                                                                    <option value="">{{ __('— Select Class —') }}</option>
                                                                    @foreach(($classes ?? collect()) as $class)
                                                                        <option value="{{ $class->id }}" {{ $studentClassId == $class->id ? 'selected' : '' }}>
                                                                            {{ $class->name }} {{ $class->code ? '(' . $class->code . ')' : '' }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            <div class="col-md-6">
                                                                <label class="form-label">{{ __('Stream') }}</label>
                                                                <select name="class_stream_id" class="form-select student-stream-select" id="studentStreamSelect{{ $user->id }}" data-user-id="{{ $user->id }}">
                                                                    <option value="">{{ __('— No Stream —') }}</option>
                                                                    @foreach(($streams ?? collect()) as $stream)
                                                                        <option value="{{ $stream->id }}" {{ $studentStreamId == $stream->id ? 'selected' : '' }} data-class-id="{{ $stream->class_id }}">
                                                                            {{ $stream->name }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                            ...
                                                            <div class="col-12">
                                                                ...
                                                                <select name="subject_ids[]" class="form-select" multiple size="8">
                                                                    ...
                                                                    @if($groupedSubjects->isEmpty())
                                                                        @foreach(($subjects ?? collect()) as $subj)
                                                                            <option value="{{ $subj->id }}" {{ in_array($subj->id, $studentSubjects) ? 'selected' : '' }}>
                                                                                {{ $subj->educationLevel?->name ? '[' . $subj->educationLevel->name . '] ' : '' }}
                                                                                {{ $subj->name }}
                                                                                {{ $subj->code ? ' (' . $subj->code . ')' : '' }}
                                                                            </option>
                                                                ...
                                                                <small class="text-muted">{{ __('Hold Ctrl/Command to select multiple subjects') }}</small>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="modal-footer">
