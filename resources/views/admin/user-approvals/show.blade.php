                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.user-approvals.approve', $user) }}" class="mb-3">
                            ...
                            <button type="submit" class="btn btn-success w-100"
                                onclick="return confirm('{{ __('Approve this user registration?') }}')">
            ...
            @if ($user->approval_status === 'approved')
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        ...
                        @else
                            <form method="POST" action="{{ route('admin.user-approvals.reinstate', $user) }}"
                                class="mb-2">
                                @csrf
                                <button type="submit" class="btn btn-success w-100"
                        ...
                        <form method="POST" action="{{ route('admin.user-approvals.expel', $user) }}">
                            ...
                            <input type="hidden" name="reason"
                                value="{{ __('Expelled/Terminated via approvals detail') }}">
                                    ...
                                    <div class="row g-3">
                                        <div class="col-12">
                                            ...
                                            <select name="employment_role" class="form-select" required>
                                                @foreach (['Teacher', 'Bursar', 'Nurse', 'Staff', 'Other'] as $role)
                                                    <option value="{{ $role }}"
                                                        {{ $user->hasRole($role) ? 'selected' : '' }}>{{ __($role) }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-12">
                                            ...
                                            <input type="text" name="employee_type" class="form-control"
                                                placeholder="{{ __('e.g., full_time, part_time, contract') }}">
                                        </div>
                                        <div class="col-12">
                                            <div class="alert alert-info mb-0">
                                                <i class="bi bi-info-circle"></i>
                                                {{ __('If role is Teacher, you can optionally assign class and subjects here.') }}
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            ...
                                            <select name="class_id" class="form-select">
                                                ...
                                                @php($classes = $classes ?? \App\Models\Academic\ClassRoom::orderBy('name')->get(['id', 'name', 'code']))
                                                @foreach ($classes as $class)
                                                    <option value="{{ $class->id }}">{{ $class->name }}
                                                        {{ $class->code ? '(' . $class->code . ')' : '' }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-12">
                                            <label class="form-label">{{ __('Subjects (optional)') }}</label>
                                            @php($subjects = $subjects ?? \App\Models\Academic\Subject::orderBy('name')->get(['id', 'name']))
