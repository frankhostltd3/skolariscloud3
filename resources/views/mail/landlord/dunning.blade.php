@php
  ...
  $vars = [
    ...
    '{{days_overdue}}' => max(0, now()->startOfDay()->diffInDays(optional($invoice?->due_at)?->startOfDay() ?? now(), false) * -1),
    '{{pay_url}}' => route('landlord.invoices.show', $invoice?->getKey() ?? 0),
    '{{termination_date}}' => e(now()->addDays(($policy?->termination_grace_days ?? 30))->toDateString()),
  ];
